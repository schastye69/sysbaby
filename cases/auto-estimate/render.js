/* =============================================================================
   ...
   which moved across nearly whole; every departure is listed in DEVIATIONS.md)
...
(function () {
  ...
  /* The phone's copy of the running total.
     On a wide screen the sum is never out of sight — the panel stands to the
     right and stays there. On a phone the panel is at the end of a form six
     thousand pixels long, so a person prices the job blind. This pill carries the
     same number as .big-total and is also the way back to it. The stylesheet
     shows it under 680 px only; app.js keeps it current and takes it off the
     screen while the panel itself is visible. */
  function buildTotalChip() {
    return el("button", {
      type: "button", id: "mobileTotal", cls: "pill total-chip",
      "aria-label": "Итого — перейти к смете"
    }, [
      el("span", { cls: "k", text: "Итого" }),
      el("span", { cls: "v", id: "mobileTotalValue", text: "0 €" })
    ]);
  }
  ...
  /* The placeholder is REPLACED, not filled.
     -------------------------------------------------------------------------
     The first version of this renderer appended everything into <div id="app">
     and left the wrapper standing. That one extra element silently destroyed the
     layout: this is a two-column design built on `body{display:flex}`, where
     .container (70%) and .panel (30%) are flex items OF THE BODY. Nested one
     level deeper they stopped being flex items and became ordinary stacked
     blocks — the summary panel, with the running total on it, fell five
     thousand pixels below the form and shrank to a third of the screen.
     Everything still worked; it simply was no longer where a person could see
     it. The docblock at the top of this file promises "the same DOM, node for
     node" — the wrapper made that promise false one node in.

     So the mount point is consumed. After this call the children of <body> are
     exactly the children the PHP page emitted, in the same order, and every
     rule written against `body > .container` still means what its author
     meant. */
  function mount() {
    ...
    if (!root) return;
    var frag = document.createDocumentFragment();
    frag.appendChild(buildContainer());
    frag.appendChild(buildPanel());
    frag.appendChild(buildModals());
    frag.appendChild(buildTotalChip());
    var parent = root.parentNode;
    parent.insertBefore(frag, root);   /* in the placeholder's exact position */
    parent.removeChild(root);          /* and then the placeholder is gone    */
