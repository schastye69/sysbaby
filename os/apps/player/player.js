...
(function () {
  ...
  if (typeof window.registerApp === "function") {
    window.registerApp("player", {
      ...
      title: "Player",
      /* ПОЧЕМУ БЕЗ ЗНАЧКА — СИСТЕМЕ, А НЕ КОММЕНТАРИЮ (D-243). Причина была
         написана честно и жила в этом файле: спросить её было нельзя. */
      why: "Проигрыватель без своей музыки — картинка проигрывателя: показать ему нечего, пока вы не дали папку. Снят со стола временно (D-199); открывается по имени.",
      /* СНЯТ СО СТОЛА ВРЕМЕННО (D-199). Проигрыватель без своей музыки —
         картинка проигрывателя. Вернуть — снять эту строку. */
      hidden: true,
