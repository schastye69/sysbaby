...
(function () {
  ...
  if (typeof window.registerApp === "function") {
    window.registerApp("viewer", {
      ...
      icon: ICON,
      size: { w: 720, h: 560 },
      /* ПОЧЕМУ БЕЗ ЗНАЧКА — СИСТЕМЕ, А НЕ КОММЕНТАРИЮ (D-243). */
      why: "Просмотр открывается вещью, а не значком: без вещи ему нечего показать, и значок обещал бы то, чего стол не выполнит.",
