...
(function () {
  ...
  /* ─────────── ЧИСЛО УХОДИТ ИЗ ТЕКСТА, КОГДА СТАЛО МЕТКОЙ (v94) ─────────
     ПОВОД, дословно от основателя 29.08.2026: «после того как пользователь
     указал в заметках на рабочем столе дату или время в таком виде дата
     xx.xx.xxxx и время xx:xx, то всё остаётся как есть, только в самой
     заметке из текста дата и время должны уходить... речь об оптимизации
     места и о бессмысленных дубликатов».

     ОДНА МЫСЛЬ: ЧИСЛО ЖИВЁТ В ОДНОМ МЕСТЕ. Пока «13:00» — просто текст,
     это текст. Как только система его ПРОЧИТАЛА и показала меткой, в двух
     местах стоит одно и то же, и одно из них лишнее. Уходит текст, потому
     что метка знает больше: она умеет сказать «завтра», сосчитать минуты
     и покраснеть.

     ГРАНИЦЫ, И ОНИ ЗДЕСЬ ГЛАВНОЕ:
       · Уходит только ЧИСЛОВАЯ запись — 31.08, 31.08.2026, 11:00. Слова
         «завтра», «31 августа», «в пятницу» остаются: они часть речи
         человека, а не дубликат.
       · Уходит только то, что система ДЕЙСТВИТЕЛЬНО прочитала. Не прочитала
         — не трогает.
       · НИКОГДА НЕ ОСТАВЛЯЕТ ПУСТО. Заметка «13:00» и есть свой час:
         вычеркнуть его значило бы стереть заметку. Остаётся как есть.
       · Не во время письма, а КОГДА ЗАМЕТКУ ОТПУСТИЛИ. Строка, исчезающая
         под пальцами, — не забота, а испуг.
       · Только на рабочем столе.

     Охраняется tools/note-strip-check.mjs. */
  ...
  window.sbNoteStrip = function (text) {
    ...
    if (window.sbNoteDue(out)) out = out.replace(DATE_LIT, "$1");
    out = out.replace(/[ \t]{2,}/g, " ")
             .replace(/\s+([,.;:!?])/g, "$1")
             .replace(/^[\s,;:–—-]+/, "")
             .replace(/[\s,;:–—-]+$/, "");
    /* Заметка, от которой осталось пусто, была ЦЕЛИКОМ про этот час.
       Тогда число — не дубликат, а вся заметка. Возвращается как было. */
    return out.trim() ? out : String(text == null ? "" : text);
  };
  ...
  function buildNote(rec) {
    ...
    ta.addEventListener("blur", stripOnLeave(ta));
    bodyEl.addEventListener("blur", stripOnLeave(bodyEl));
