...
(function () {
  ...
  var STRINGS = {
    en: {
      ...
      "cc.transparency": "Reduce transparency",
      "cc.volume": "Volume", "cc.brightness": "Brightness", "cc.language": "Language",
      "cc.fullscreen": "Full screen", "cc.wallpaper": "Wallpaper", "cc.controls": "Window controls", "cc.controls.left": "Left", "cc.controls.right": "Right",
      "cc.shortcuts": "Keyboard Shortcuts", "cc.layouts": "Workspace Layouts",
      "cc.health": "Health", "cc.incognito": "Incognito Desktop",
      ...
      "note.placeholder": "Write something…", "note.sheetQ": "What matters now?",
      "note.aria": "Note",
      ...
      "note.due.rightNow": "now", "note.due.inMin": "in {n} min", "note.due.endOf": "by end of {day}",
      ...
      "h.measured": "Live, measured — not estimated",
      /* --- command palette --- */
      ...
      "aria.minimizeAll": "sys.baby private OS — clear the desk: minimise every window",
      "aria.tip": "Show a tip", "cc.tip": "Show a tip",
      "aria.cc": "Control Center",
      ...
      "net.gone": "No internet", "net.nothingChanged": "Nothing has changed. Everything you have written is here, because it was always here. Only Browser needs the network — other people's pages are not ours to keep.", "net.stillHere": "No network — the system is here", "cc.connection": "Connection", "cc.online": "Online",
      ...
      "lock.duress": "Duress word", "lock.duressOff": "Remove duress word",
      "lock.duressField": "a second word — opens another system",
      "lock.duressWhat": "A second word opens a different, complete system: its own desk, its own notes. Nothing tells anyone whether that word exists — not a label, not a delay, not one line on disk. That silence is the only reason it works.", "lock.stays": "Stays readable on disk even while locked — and only this:", "lock.stays.sysbaby.lock.v1": "the lock record itself: its salt and cipher names — without them the key cannot be derived", "lock.stays.sysbaby.activeProfile": "which profile of this browser to open — a number, not a name", "lock.stays.sysbaby.authed": "a yes-or-no mark that sign-in was passed", "lock.stays.folder": "the pointer to your backup folder, with the folder's name — the copies in it are sealed", "lock.keyFile": "Second key file", "lock.keyFileWhat": "This lock also needs its second key file — to change the word, set the second word, make a recovery code or remove the lock.", "lock.spare": "Recovery code", "lock.spareWhat": "A code of 32 characters opens your main system if you forget the word — even without the second key file. It is shown once. Write it on paper and keep it away from this device: whoever holds the code holds the system. A new code cancels the old one; a used code stops working.", "lock.spareOn": "Recovery code made on {d}.", "lock.spareNone": "No recovery code yet.", "lock.spareDone": "Write it down now — it will not be shown again.", "lock.spareSave": "Save as a file", "lock.spareFile": "sys.baby — recovery code\n\n{code}\n\nMade on {d}. It opens your main system on this device if you forget the word — without the word and without the second key. It works once. Keep it away from the device: whoever holds it holds the system.\n", "lock.forgot": "Forgot the word?", "lock.codeWhat": "Enter the recovery code and a new word. The code opens your main system and is then used up. If a second key was set, it is removed, and a second word made with it is lost.", "lock.code": "Recovery code", "lock.codeNew": "New word", "lock.codeAgain": "New word again", "lock.codeOpen": "Open with the code", "lock.codeWrong": "This code does not open the lock. Nothing has changed.", "lock.codeUsed": "The recovery code is used up. Make a new one in your account window.",
      "lock.duressSame": "the second word must differ from the first", "lock.duressDone": "Done.",
      "lock.costMem": "Plus {mb} MiB of memory for every guess — the part graphics cards and custom chips cannot share.",
      "lock.strong": "Strengthen with memory",
      "lock.strongWhat": "This lock stretches your password with time only. Strengthening adds Argon2id: every guess will also need 64 MiB of its own memory, which makes guessing on graphics cards and custom chips many times more expensive. Your notes stay as they are — only the doors are rebuilt. If you set a duress word, type it when asked: without it, the system behind it will be lost.",
      "lock.strongAsk": "If you set a duress word, type it here and press again. Leave it empty if you did not — the second door will become noise.",
      "lock.strongDone": "Strengthened: every guess now costs memory as well as time.",
      "lock.duressWrong": "That duress word does not open its door — nothing was changed.",
      "lock.hw": "Device key",
      "lock.hwWhat": "Bind the lock to the secure chip of this device. After that it opens only with your password AND your fingerprint, face or device PIN. The secret never leaves the chip: the system receives only its answer to one question, and that answer is written nowhere. A stolen disk plus a peeked password is still nothing. If the device is lost, only the recovery code opens the system — and it removes the device key.",
      "lock.hwOn": "This lock also asks the device key.",
      "lock.hwNone": "No device key is bound.",
      "lock.hwCant": "This browser cannot hold a device key here.",
      "lock.hwAdd": "Bind device key",
      "lock.hwOff": "Unbind device key",
      "lock.hwDone": "Bound. From now on the lock asks your device as well.",
      "lock.hwRemoved": "Unbound. The lock opens with the password again.",
      "lock.hwNoPrf": "This browser or device cannot answer the question the lock needs (WebAuthn PRF). Nothing was changed.",
      "lock.hwFailed": "The device did not confirm — nothing was changed.",
      "lock.hwNeed": "This lock also asks your device: after the password, confirm with fingerprint, face or PIN.",
      "lock.hwNo": "The device key did not answer — the lock stays shut. Without the device only the recovery code opens it.",
      "mood.studio": "Studio", "mood.aurora": "Aurora", "mood.sunset": "Sunset", "mood.ocean": "Ocean", "mood.mono": "Mono", "mood.daylight": "Daylight", "mood.therapy": "Light therapy", "mood.feast": "Feast", "mood.ember": "Ember", "mood.own": "Yours", "mood.midnight": "Midnight", "mood.garden": "Garden", "mood.lavender": "Lavender", "mood.honey": "Honey", "menu.tidyDesk": "Tidy desk", "menu.bringThing": "Bring a thing\u2026", "menu.hideFab": "Remove this button", "part.fab": "Quick actions button", "br.back": "Back", "br.fwd": "Forward", "br.again": "Reload", "br.bookmark": "Save this page", "br.bookmarked": "Saved", "br.openTab": "Open in a real tab", "br.placeholder": "Address, or something to look up", "br.frameTitle": "Page", "br.saved": "Saved", "br.recent": "Recent", "br.empty.title": "Nothing open yet", "br.empty.sub": "Type an address and press Enter. Many sites refuse to be shown inside another page \u2014 the arrow at the right opens the current address in a real tab, always.", "note.day.title": "That day is yours", "note.day.body": "The date is written down \u2014 keep typing what happens on it.", "lock.again": "Repeat the password", "lock.mismatch": "The two do not match \u2014 nothing was locked.", "lock.failed": "Could not lock \u2014 nothing was changed.", "lock.remove": "Remove the lock", "lock.removed": "The lock is off. Your words lie open again.", "lock.warn": "If you forget this password and have no recovery code, what you have written cannot be recovered. No server holds a copy — that is the whole point. Make a recovery code below and keep it on paper.", "lock.key": "Second key — choose the file", "lock.keyNeed": "This system also asks for its second key. Without the file the password alone derives nothing.", "alarm.title": "Alarm", "alarm.now": "{time} — now.", "alarm.passed": "{time} passed {ago}.", "alarm.agoMin": "{n} min ago", "alarm.agoHour": "{n} h ago", "alarm.agoDay": "{n} days ago", "alarm.set": "Alarm set", "alarm.off": "Alarm off", "alarm.arm": "Wake me at this time", "set.desktop.wake": "Wake me outside the tab", "set.desktop.wakeSub": "The only thing that ever leaves this system. Turning it on asks the browser for permission; the alarm then shows up in your phone\u2019s notifications — where anyone standing nearby can read it. Off by default, and it stays off until you say otherwise.", "lock.cost": "{n} rounds · {ms} ms. That is what ONE guess costs — and they need millions.", "lock.sealed": "{n} envelopes on this disk. Not one of them can be read from here.", "lock.ask": "Password", "lock.open": "Open", "lock.wrong": "That password does not open this.", "lock.locked": "Locked", "lock.title": "Lock everything", "lock.insecure": "The lock cannot work at this address. A browser gives a page real cryptography only over a protected connection (https). Open sys.baby over https and the lock appears — nothing else changes.", "aria.turboOn": "Turbo is on — tap to give the frames back", "cc.turbo": "Turbo", "lock.state.none": "Not locked", "lock.state.armed": "Locked — open for this session", "lock.what": "One password closes the whole system: every note, every letter, every conversation, the Vault, and the look of the room. On disk nothing readable is left — not even the names of the things you keep.", "lock.accounts": "Several accounts? One password opens all of them on this device. The lock stands in front of the storage, not in front of one account — and while it is shut, even the number of your accounts is invisible.", "lock.new": "Password", "lock.set": "Lock everything", "lock.now": "Lock now", "lock.idle": "Lock when idle for", "lock.idle.never": "Never", "lock.idle.min": "{n} min", "lock.idle.hour": "{n} h", "lock.idleWhat": "No click, key, wheel or touch for this long \u2014 and the session closes: the door returns, the words stay in their envelopes.", "lock.change": "Change the password", "lock.changed": "The password is changed. Nothing was re-encrypted — only the envelope that holds the key.", "lock.old": "Current password", "lock.short": "At least four characters.", "lock.busy": "Working — this takes a second on purpose.", "auth.lock.title": "All of this can be closed with one word", "auth.lock.body": "sys.baby has no server. What you write stays on this device — which means nobody can read it for you, and nobody can read it instead of you. One turn of the key and the whole system goes into envelopes: notes, letters, conversations, even the colour of your walls. Nobody else can recover it — nobody holds a copy; what can save you is your own recovery code, on paper, which the lock offers. You can do it now, or on any later day — the aperture in the top bar is always there.", "auth.lock.now": "Set it now", "auth.lock.later": "Later, from the top bar", "bk.title": "Copies", "bk.folder": "Folder", "bk.noFolder": "not chosen", "bk.last": "Last copy", "bk.never": "none yet", "bk.sealed": "sealed", "bk.choose": "Choose a folder", "bk.change": "Change the folder", "bk.on": "On", "bk.off": "Off", "bk.saveNow": "Save a copy now", "bk.exportNow": "Download a copy as a file", "bk.err": "Error:", "bk.what": "The system puts a copy of your profile into a folder you choose — every time something changed, and at the moment you leave the page. Three generations are kept: trouble noticed a day later can only be undone with yesterday's copy. While the lock is on, the copy goes into the folder SEALED and opens with the same password.", "bk.needFolder": "Without a real folder the synchronisation does not switch on. A copy living in the same browser as the original is not a copy: one clearing of the browser takes both.", "bk.unsupported": "This browser does not let a page write into a folder on your device, so there is no synchronisation here — and the Council will not pretend otherwise. Saving copies into the same browser would be easy and would be a lie: a copy living in the same browser as the original is not a copy, because one clearing of the browser takes both. What remains is a file you download yourself, with the button below. On desktop Chrome or Edge the synchronisation switches on by itself.", "bk.permission": "The browser wants you to confirm access to the folder. Press “Save a copy now” once more and allow it.", "panel.account": "Account", "aria.identity": "Account and leaving", "acc.guest": "a guest on this machine", "acc.name": "Name in the system", "acc.nameSub": "Shown in the top bar and on what you send. Two characters at least.", "acc.save": "Save", "acc.saved": "Saved.", "acc.profile": "Profile", "acc.signout": "Sign out, softly", "acc.leave": "Leave now", "acc.leaveSub": "One tap: the session closes, every cache and service copy is erased, the address and title of this history entry are replaced, and the tab tries to close. What you have written stays where it is — and stays sealed if the lock is on.", "acc.wipe": "Erase everything and leave", "acc.wipeSub": "All of the above, plus everything you have written: notes, letters, conversations, the Vault, profiles. Nothing of yours is left on this device. There is no undo.", "acc.wipeAsk": "Press once more to erase everything. This cannot be undone.", "acc.truth": "What a page cannot do, and the Council will not pretend otherwise: the browser's list of visits is not this page's memory, and no site may read or erase it — a site that could erase itself from your history could erase anything else too. This page replaces the entry it can reach, so Back will not lead here, and erases everything of its own. If you need visits not to be recorded at all, use a private window; to remove what is already recorded, clear it in the browser itself.", "panel.time": "Time & calendar", "aria.time": "Time and calendar", "pl.prev": "Previous", "pl.next": "Next", "pl.play": "Play", "pl.pause": "Pause", "pl.seek": "Position", "pl.pick": "Choose a folder", "pl.count": "{n} tracks", "pl.brought": "Music", "pl.broughtN": "{n} tracks brought in", "pl.none": "No audio files in that folder", "pl.empty.title": "No music yet", "pl.empty.sub": "A browser cannot look through your device on its own. Choose ONE folder \u2014 Music or Download, for example. Android refuses the storage root itself: if it says \u201cCan\u2019t use this folder\u201d, step into a folder and choose that.", "br.blank": "Page blank?", "br.blank.go": "Open in a tab", "br.shut.sub": "You told the system this site refuses to be shown inside another page. It opens in a real tab from now on \u2014 and this can be undone.", "br.shut.retry": "Try here again",
      /* ================================================== Pulse / Настройки
      ...
      "ml.compose.honestStudio": "Send posts your letter through {host} — not our server. Your words are readable there before they reach the studio inbox. To go around it, write straight to build@sys.baby.",
      ...
      "ml.status.emptyBody": "Write something first — an empty letter says nothing.", "ml.compose.sendOwn": "Your own mail", "ml.compose.ownNote": "«Your own mail» opens your mail app with this letter: it leaves through your own mail service, as plain text, not through sys.baby.", "ml.status.ownTo": "Your own mail needs a real address, like name@example.com.", "ml.tag.handed": "your mail", "ml.delivery.handed": "Handed to your own mail app on {when} — it sends the letter, not this system; whether it arrived, this system does not know.", "ml.toast.handedTitle": "Handed to your mail", "ml.toast.handedBody": "Your mail app opened with the letter. It sends it — a copy stays here in Sent.",
      ...
      "fv.hand.none": "Nothing chosen",
      "fv.hand.noneSub": "Pick a file first — a folder cannot be handed over as one thing.",
      ...
      /* ============================================== Whisper / Разговор */
      "mg.sampleNotice": "Nothing was sent anywhere: this system does not carry messages itself. Set a shared word with this person — then a sealed message can travel any way you like, and only they can open it.", "mg.pact": "Shared word", "mg.pactSet": "Shared word ✓", "mg.pactWhat": "Agree on a shared word with this person — in person or by voice, never through the same channel. It seals your messages: whoever carries them — a messenger, e-mail, SMS — sees only noise. The word stays under your lock and is never sent.", "mg.pactPh": "shared word, 6 characters or more", "mg.pactSave": "Keep the word", "mg.pactOn": "Messages here can now be sealed with your shared word.", "mg.pactShort": "The shared word must be at least {n} characters.", "mg.seal": "Seal", "mg.sealTitle": "Seal this message to carry it any way you like", "mg.sealFailed": "This browser could not seal the message — nothing was pretended.", "mg.sealedOut": "Sealed. Send this text any way you like — only your shared word opens it:", "mg.copy": "Copy", "mg.copied": "Copied.", "mg.copyHand": "Selected — copy it by hand.", "mg.share": "Share", "mg.close": "Close", "mg.paste": "Open a sealed message", "mg.pasteWhat": "Paste a sealed message you received from this person — spaces and words around it do not matter.", "mg.pasteOpen": "Open", "mg.pasteWrong": "This does not open with your shared word — nothing was added.", "mg.pasteNoPact": "Set the shared word first — without it there is nothing to open with.", "mg.pasteNone": "There is no sealed message (sb1:…) in what you pasted.", "mg.sealedTag": "sealed",
      ...
      "hand.to": "Hand to…",
      /* ── ВНУТРЕННИЙ ЯЗЫК СИСТЕМЫ ПЕРЕВОДИТСЯ НА ЭКРАНЕ · D-253 ───────────
         Роды вещей, нужды комнат, состояния, слова загрузки и срок подбора
         пароля — всё это система называет по-русски внутри себя, и это её
         право. Но на экран это выходило как есть: «89858 млрд лет» посреди
         английской фразы, «21 вещи · 28 выбор» в английском терминале.
         Основатель: «такого нигде не должно быть». Внутреннее слово идёт на
         экран только через этот словарь. Охраняется one-alphabet-check. */
      "kind.вещи": "things", "kind.выбор": "choices", "kind.служебное": "housekeeping",
      "need.диск": "disk", "need.сеть": "network", "need.папка": "folder", "need.почтальон": "postman",
      "need.свой сервер": "own server", "need.чужое согласие": "the other site's consent", "need.собеседник": "a live person",
      "verdict.есть": "yes", "verdict.нет": "no", "verdict.нечем проверить": "nothing here can check",
      "state.живо": "working", "state.ждёт": "waiting", "state.нечем проверить": "unverifiable",
      "who.ядро": "core", "how.чтение": "read", "how.запись": "write",
      "why.solitaire": "A gift, not a tool: the chest opens this room, and until it is given the desk is no poorer without it.",
      "boot.records": "{n} records", "boot.registered": "{n} registered", "boot.works": "{n} works",
      "boot.priceOk": "price list in place", "boot.priceNo": "no price list",
      "boot.storageShut": "storage is shut by the browser", "boot.noApps": "no application registered", "boot.noPortfolio": "portfolio did not load",
      "years.b": "{n} billion years", "years.m": "{n} million years", "years.y": "{n} years", "years.lt1": "under a year",
      /* Объяснения приборов «что живо» (D-243) и причины комнат без значка. */
      "alive.disk.noPlace": "this room has no place of its own on the disk: nowhere to write",
      "alive.disk.noRights": "the rights core did not come up, and there is nobody to ask for the room's box",
      "alive.disk.empty": "the place is empty: checked with a trial write, then removed",
      "alive.disk.same": "written and read back just now, and what lies there is what lay there before",
      "alive.disk.refused": "the write did not come back: storage is shut or rights do not allow it",
      "alive.net.unknown": "the browser said nothing about the network",
      "alive.net.on": "the cable is plugged in. whether whoever you need answers is not visible from here",
      "alive.net.off": "the browser says there is no network",
      "alive.post.noList": "the outward register did not load, and there is nobody to ask for the address",
      "alive.post.none": "the outward register names no outside owner at all",
      "alive.post.named": "the address is named in the outward register: {hosts}. whether it accepts a letter today is not visible from here",
      "alive.folder.yes": "the browser can open a folder of your choosing",
      "alive.folder.no": "this browser cannot open a folder: work is limited to what was dragged in",
      "alive.server.noList": "the outward register did not load, and there is nobody to ask",
      "alive.server.none": "no door in the outward register declares a way back: there is no address today at which anything would reach you",
      "alive.server.named": "a way back is declared in the outward register: {hosts}",
      "alive.consent": "whether another site agrees to be shown in a frame is learned only by trying to open it, and the system does not go outside to talk about itself",
      "alive.peer": "a live person on the other end is measured by nothing here: silent and absent look the same from inside",
      "alive.noProbe": "the system has no instrument for this need, and it will not guess",
      "alive.probeFailed": "the instrument did not run: {err}",
      "why.browser": "A page inside a page: most sites refuse to be shown this way, and the rest need the network — the very thing the system is built to live without.",
      "why.experimental": "A bench, not a tool: it is where we check whether a thing is possible. It gives a person nothing at this stage.",
      "why.mail": "Without a server, letters have nowhere to go and nowhere to come from: the app shows how it WOULD be, not the work itself. The postman's address is in the outward register; a server that would receive letters for you does not exist.",
      "why.messenger": "Messages travel by your own hand: sealed with a shared word, carried any way you like. Whether a live person is on the other end, nothing here can measure.",
      "why.player": "A player without its own music is a picture of a player: it has nothing to show until you hand it a folder. Taken off the desk for now (D-199); opens by name.",
      "why.project": "The live system opens from work, not from an icon: a portfolio card, a linked letter, a brief in the Vault. An icon on the desk would lead into an empty frame.",
      "why.viewer": "The viewer opens from a thing, not from an icon: without a thing it has nothing to show, and an icon would promise what the desk cannot deliver.",
      "hand.done": "Handed over", "hand.src.from": "Copied from «{room}» on {d}.", "hand.src.same": "It matches the source.", "hand.src.changed": "The source has changed since.", "hand.src.gone": "The source is gone — this is the last copy.", "hand.src.update": "Take the newer version",
      "hand.doneSub": "It is there now. It stays here too.",
      "hand.fail": "Not handed over",
      "hand.failSub": "That room did not take it, and nothing pretends it did.",
      "nt.ph.title": "Title", "nt.ph.body": "Start typing…",
      "nt.ph.search": "Search notes", "nt.new": "New note",
      "nt.emptyList": "Nothing found", "nt.more": "Show {n} more",
      "nt.empty": "No notes yet. Write one and it stays with you \u2014 not a line of it leaves.",
      "nt.newNote": "New note",
      "ec.files.one": "1 deleted file", "ec.files.many": "{n} deleted files", "ec.file.folder": "folder", "ec.file.file": "file", "ec.silenceAsk": "Delete this for good? There is no way back.", "ec.restore": "Restore",
    ...
    ru: {
      ...
      "cc.transparency": "Меньше прозрачности",
      "cc.volume": "Громкость", "cc.brightness": "Яркость", "cc.language": "Язык",
      "cc.fullscreen": "\u0412\u043e \u0432\u0435\u0441\u044c \u044d\u043a\u0440\u0430\u043d", "cc.wallpaper": "Обои", "cc.controls": "Кнопки окон", "cc.controls.left": "Слева", "cc.controls.right": "Справа",
      "cc.shortcuts": "Горячие клавиши", "cc.layouts": "Раскладки окон",
      "cc.health": "Диагностика", "cc.incognito": "Скрытый рабочий стол",
      ...
      "note.placeholder": "Напишите что-нибудь…", "note.sheetQ": "Что сейчас важно?",
      "note.aria": "Заметка",
      ...
      "note.due.rightNow": "сейчас", "note.due.inMin": "через {n} мин", "note.due.endOf": "до конца {day}",
      ...
      "toast.closed": "Окно «{app}» закрыто", "toast.closedBody": "Нажмите «Отменить», чтобы вернуть как было.",
      "toast.undo": "Отменить",
      ...
      "p.saveCurrent": "Сохранить текущую",
      ...
      "h.measured": "Вживую, измерено — не оценка",
      ...
      "aria.minimizeAll": "sys.baby private OS — убрать со стола: свернуть все окна",
      "aria.tip": "Показать подсказку", "cc.tip": "Показать подсказку",
      "aria.cc": "Центр управления",
      ...
      "net.gone": "Интернета нет", "net.nothingChanged": "Ничего не изменилось. Всё написанное здесь, потому что оно всегда было здесь. Сеть нужна только Браузеру — чужие страницы не наши, чтобы их хранить.", "net.stillHere": "Сети нет — система на месте", "cc.connection": "Соединение", "cc.online": "В сети",
      ...
      "lock.duress": "Тревожное слово", "lock.duressOff": "Снять тревожное слово",
      "lock.duressField": "второе слово — откроет другую систему",
      "lock.duressWhat": "Второе слово открывает другую, целую систему: свой стол, свои записи. Ничто не говорит, заведено оно или нет, — ни надпись, ни задержка, ни строка на диске. Это молчание и есть единственная причина, по которой оно работает.", "lock.stays": "При запертом замке на диске остаётся читаемым только это:", "lock.stays.sysbaby.lock.v1": "сама запись замка: соль и названия шифров — без них ключ не вывести", "lock.stays.sysbaby.activeProfile": "какой профиль этого браузера открывать — номер, а не имя", "lock.stays.sysbaby.authed": "отметка «вход пройден» — да или нет", "lock.stays.folder": "указатель на папку копий вместе с её именем — сами копии в ней запечатаны", "lock.keyFile": "Файл второго ключа", "lock.keyFileWhat": "Этому замку нужен и файл второго ключа — чтобы сменить слово, завести второе слово, код восстановления или снять замок.", "lock.spare": "Код восстановления", "lock.spareWhat": "Код из 32 знаков открывает главную систему, если слово забыто, — даже без файла второго ключа. Показывается один раз. Запишите его на бумаге и храните отдельно от этого устройства: у кого код — у того система. Новый код отменяет прежний; использованный больше не работает.", "lock.spareOn": "Код восстановления заведён {d}.", "lock.spareNone": "Кода восстановления пока нет.", "lock.spareDone": "Запишите сейчас — второй раз он показан не будет.", "lock.spareSave": "Сохранить файлом", "lock.spareFile": "sys.baby — код восстановления\n\n{code}\n\nЗаведён {d}. Открывает главную систему на этом устройстве, если слово забыто, — без слова и без второго ключа. Работает один раз. Храните отдельно от устройства: у кого код — у того система.\n", "lock.forgot": "Забыли слово?", "lock.codeWhat": "Введите код восстановления и новое слово. Код откроет главную систему и после этого будет израсходован. Если был второй ключ, он снимется, а второе слово, заведённое с ним, потеряется.", "lock.code": "Код восстановления", "lock.codeNew": "Новое слово", "lock.codeAgain": "Новое слово ещё раз", "lock.codeOpen": "Открыть кодом", "lock.codeWrong": "Этот код замок не открывает. Ничего не изменилось.", "lock.codeUsed": "Код восстановления израсходован. Заведите новый в окне аккаунта.",
      "lock.duressSame": "второе слово должно отличаться от первого", "lock.duressDone": "Готово.",
      "lock.costMem": "И {mb} МиБ памяти на каждую попытку — то, чего видеокарты и заказные микросхемы не могут делить.",
      "lock.strong": "Усилить памятью",
      "lock.strongWhat": "Этот замок растягивает пароль только временем. Усиление добавит Argon2id: каждой попытке подобрать пароль понадобятся ещё и 64 МиБ своей памяти — перебор на видеокартах и заказных микросхемах станет многократно дороже. Записи останутся как есть — переделываются только двери. Если вы заводили тревожное слово, впишите его, когда попросят: без него система за ним будет потеряна.",
      "lock.strongAsk": "Если заводили тревожное слово — впишите его сюда и нажмите ещё раз. Не заводили — оставьте пустым: вторая дверь станет шумом.",
      "lock.strongDone": "Усилено: теперь каждая попытка стоит не только времени, но и памяти.",
      "lock.duressWrong": "Это тревожное слово не открывает свою дверь — ничего не изменено.",
      "lock.hw": "Ключ устройства",
      "lock.hwWhat": "Привяжите замок к защищённому чипу этого устройства. После этого он открывается только паролем И вашим отпечатком, лицом или PIN устройства. Секрет никогда не покидает чип: система получает лишь его ответ на один вопрос, и этот ответ нигде не записывается. Украденный диск вместе с подсмотренным паролем — всё ещё ничего. Потеряете устройство — систему откроет только код восстановления, и он снимет ключ устройства.",
      "lock.hwOn": "Этот замок спрашивает и ключ устройства.",
      "lock.hwNone": "Ключ устройства не привязан.",
      "lock.hwCant": "Этот браузер не умеет держать здесь ключ устройства.",
      "lock.hwAdd": "Привязать ключ устройства",
      "lock.hwOff": "Отвязать ключ устройства",
      "lock.hwDone": "Привязано. Теперь замок спрашивает и ваше устройство.",
      "lock.hwRemoved": "Отвязано. Замок снова открывается паролем.",
      "lock.hwNoPrf": "Этот браузер или устройство не умеет отвечать на вопрос, нужный замку (WebAuthn PRF). Ничего не изменено.",
      "lock.hwFailed": "Устройство не подтвердило — ничего не изменено.",
      "lock.hwNeed": "Этот замок спрашивает и устройство: после пароля подтвердите отпечатком, лицом или PIN.",
      "lock.hwNo": "Ключ устройства не ответил — замок остаётся закрытым. Без устройства его откроет только код восстановления.",
      "mood.studio": "Студия", "mood.aurora": "Аврора", "mood.sunset": "Закат", "mood.ocean": "Океан", "mood.mono": "Моно", "mood.daylight": "Дневной свет", "mood.therapy": "Светотерапия", "mood.feast": "Праздник", "mood.ember": "Угли", "mood.own": "Своя", "mood.midnight": "Полночь", "mood.garden": "Сад", "mood.lavender": "Лаванда", "mood.honey": "Мёд", "menu.tidyDesk": "\u041f\u0440\u0438\u0431\u0440\u0430\u0442\u044c \u0441\u0442\u043e\u043b", "menu.bringThing": "\u041f\u0440\u0438\u043d\u0435\u0441\u0442\u0438 \u0432\u0435\u0449\u044c\u2026", "menu.hideFab": "\u0423\u0431\u0440\u0430\u0442\u044c \u044d\u0442\u0443 \u043a\u043d\u043e\u043f\u043a\u0443", "part.fab": "\u041a\u043d\u043e\u043f\u043a\u0430 \u0431\u044b\u0441\u0442\u0440\u044b\u0445 \u0434\u0435\u0439\u0441\u0442\u0432\u0438\u0439", "br.back": "\u041d\u0430\u0437\u0430\u0434", "br.fwd": "\u0412\u043f\u0435\u0440\u0451\u0434", "br.again": "\u041e\u0431\u043d\u043e\u0432\u0438\u0442\u044c", "br.bookmark": "\u0421\u043e\u0445\u0440\u0430\u043d\u0438\u0442\u044c \u0441\u0442\u0440\u0430\u043d\u0438\u0446\u0443", "br.bookmarked": "\u0421\u043e\u0445\u0440\u0430\u043d\u0435\u043d\u043e", "br.openTab": "\u041e\u0442\u043a\u0440\u044b\u0442\u044c \u043d\u0430\u0441\u0442\u043e\u044f\u0449\u0435\u0439 \u0432\u043a\u043b\u0430\u0434\u043a\u043e\u0439", "br.placeholder": "\u0410\u0434\u0440\u0435\u0441 \u0438\u043b\u0438 \u0447\u0442\u043e \u043d\u0430\u0439\u0442\u0438", "br.frameTitle": "\u0421\u0442\u0440\u0430\u043d\u0438\u0446\u0430", "br.saved": "\u0421\u043e\u0445\u0440\u0430\u043d\u0451\u043d\u043d\u043e\u0435", "br.recent": "\u041d\u0435\u0434\u0430\u0432\u043d\u0435\u0435", "br.empty.title": "\u041f\u043e\u043a\u0430 \u043d\u0438\u0447\u0435\u0433\u043e \u043d\u0435 \u043e\u0442\u043a\u0440\u044b\u0442\u043e", "br.empty.sub": "\u0412\u0432\u0435\u0434\u0438\u0442\u0435 \u0430\u0434\u0440\u0435\u0441 \u0438 \u043d\u0430\u0436\u043c\u0438\u0442\u0435 \u0412\u0432\u043e\u0434. \u041c\u043d\u043e\u0433\u0438\u0435 \u0441\u0430\u0439\u0442\u044b \u0437\u0430\u043f\u0440\u0435\u0449\u0430\u044e\u0442 \u043f\u043e\u043a\u0430\u0437\u044b\u0432\u0430\u0442\u044c \u0441\u0435\u0431\u044f \u0432\u043d\u0443\u0442\u0440\u0438 \u0434\u0440\u0443\u0433\u043e\u0439 \u0441\u0442\u0440\u0430\u043d\u0438\u0446\u044b \u2014 \u0441\u0442\u0440\u0435\u043b\u043a\u0430 \u0441\u043f\u0440\u0430\u0432\u0430 \u0432\u0441\u0435\u0433\u0434\u0430 \u043e\u0442\u043a\u0440\u043e\u0435\u0442 \u0430\u0434\u0440\u0435\u0441 \u043d\u0430\u0441\u0442\u043e\u044f\u0449\u0435\u0439 \u0432\u043a\u043b\u0430\u0434\u043a\u043e\u0439.", "note.day.title": "\u042d\u0442\u043e\u0442 \u0434\u0435\u043d\u044c \u2014 \u0432\u0430\u0448", "note.day.body": "\u0414\u0430\u0442\u0430 \u0443\u0436\u0435 \u0437\u0430\u043f\u0438\u0441\u0430\u043d\u0430 \u2014 \u043f\u0438\u0448\u0438\u0442\u0435 \u0434\u0430\u043b\u044c\u0448\u0435, \u0447\u0442\u043e \u0432 \u044d\u0442\u043e\u0442 \u0434\u0435\u043d\u044c \u0431\u0443\u0434\u0435\u0442.", "lock.again": "\u041f\u043e\u0432\u0442\u043e\u0440\u0438\u0442\u0435 \u043f\u0430\u0440\u043e\u043b\u044c", "lock.mismatch": "\u041f\u0430\u0440\u043e\u043b\u0438 \u043d\u0435 \u0441\u043e\u0432\u043f\u0430\u043b\u0438 \u2014 \u043d\u0438\u0447\u0435\u0433\u043e \u043d\u0435 \u0437\u0430\u043f\u0435\u0440\u0442\u043e.", "lock.failed": "\u0417\u0430\u043f\u0435\u0440\u0435\u0442\u044c \u043d\u0435 \u0432\u044b\u0448\u043b\u043e \u2014 \u043d\u0438\u0447\u0435\u0433\u043e \u043d\u0435 \u0438\u0437\u043c\u0435\u043d\u0435\u043d\u043e.", "lock.remove": "\u0421\u043d\u044f\u0442\u044c \u0437\u0430\u043c\u043e\u043a", "lock.removed": "\u0417\u0430\u043c\u043e\u043a \u0441\u043d\u044f\u0442. \u0421\u043b\u043e\u0432\u0430 \u0441\u043d\u043e\u0432\u0430 \u043b\u0435\u0436\u0430\u0442 \u043e\u0442\u043a\u0440\u044b\u0442\u043e.", "lock.warn": "Если вы забудете этот пароль и у вас не будет кода восстановления, написанное вернуть будет нельзя. Копии нет ни на одном сервере — в этом весь смысл. Заведите код восстановления ниже и храните его на бумаге.", "lock.key": "Второй ключ — выберите файл", "lock.keyNeed": "Эта система спрашивает ещё и второй ключ. Без файла из одного пароля ключ не выводится.", "alarm.title": "Будильник", "alarm.now": "{time} — сейчас.", "alarm.passed": "{time} прошло {ago}.", "alarm.agoMin": "{n} мин назад", "alarm.agoHour": "{n} ч назад", "alarm.agoDay": "{n} дн назад", "alarm.set": "Будильник поставлен", "alarm.off": "Будильник снят", "alarm.arm": "Разбудить в это время", "set.desktop.wake": "Будить вне вкладки", "set.desktop.wakeSub": "Единственное, что вообще уходит из этой системы наружу. Включение просит разрешение у браузера; дальше будильник показывается в уведомлениях телефона — там, где его прочтёт любой, кто рядом. По умолчанию выключено и само не включается.", "lock.cost": "{n} проходов · {ms} мс. Столько стоит ОДНА попытка — а их нужны миллионы.", "lock.sealed": "{n} конвертов на этом диске. Ни один из них отсюда не читается.", "lock.ask": "\u041f\u0430\u0440\u043e\u043b\u044c", "lock.open": "\u041e\u0442\u043a\u0440\u044b\u0442\u044c", "lock.wrong": "\u042d\u0442\u043e\u0442 \u043f\u0430\u0440\u043e\u043b\u044c \u043d\u0435 \u043e\u0442\u043a\u0440\u044b\u0432\u0430\u0435\u0442.", "lock.locked": "\u0417\u0430\u043f\u0435\u0440\u0442\u043e", "lock.title": "Запереть всё", "lock.insecure": "На этом адресе замок работать не может. Настоящую криптографию браузер даёт странице только по защищённому соединению (https). Откройте sys.baby по https — замок появится, и больше ничего не изменится.", "aria.turboOn": "Турбо включён — нажмите, чтобы вернуть кадры", "cc.turbo": "Турбо", "lock.state.none": "Не заперто", "lock.state.armed": "Заперто — открыто на этот сеанс", "lock.what": "Один пароль закрывает всю систему: каждую заметку, каждое письмо, каждый разговор, Хранилище и вид комнаты. На диске не остаётся ничего читаемого — даже имён того, что вы храните.", "lock.accounts": "Несколько аккаунтов? Один пароль открывает их все на этом устройстве. Замок стоит перед хранилищем, а не перед одним аккаунтом, — и пока он заперт, не видно даже, сколько у вас аккаунтов.", "lock.new": "Пароль", "lock.set": "Запереть всё", "lock.now": "Запереть сейчас", "lock.idle": "Запирать без движения через", "lock.idle.never": "Никогда", "lock.idle.min": "{n} мин", "lock.idle.hour": "{n} ч", "lock.idleWhat": "Ни нажатия, ни клавиши, ни колеса, ни касания столько времени — и сеанс закрывается: дверь возвращается, слова остаются в конвертах.", "lock.change": "Сменить пароль", "lock.changed": "Пароль сменён. Ничего не перешифровывалось — переклеен конверт с ключом.", "lock.old": "Текущий пароль", "lock.short": "Не короче четырёх знаков.", "lock.busy": "Считаю — эта задержка здесь нарочно.", "auth.lock.title": "Всё это можно закрыть одним словом", "auth.lock.body": "У sys.baby нет сервера. Написанное остаётся на этом устройстве — значит, никто не прочтёт его за вас и никто не прочтёт его вместо вас. Один поворот ключа — и вся система уходит в конверты: заметки, письма, разговоры, даже цвет ваших стен. Восстановить за вас некому — копии ни у кого нет; спасти может только ваш собственный код восстановления на бумаге, его предложит замок. Это можно сделать сейчас, а можно в любой другой день — диафрагма в верхней панели стоит там всегда.", "auth.lock.now": "Поставить сейчас", "auth.lock.later": "Потом, из верхней панели", "bk.title": "Копии", "bk.folder": "Папка", "bk.noFolder": "не выбрана", "bk.last": "Последняя копия", "bk.never": "ещё не было", "bk.sealed": "заперта", "bk.choose": "Выбрать папку", "bk.change": "Сменить папку", "bk.on": "Включена", "bk.off": "Выключена", "bk.saveNow": "Сохранить сейчас", "bk.exportNow": "Скачать копию файлом", "bk.err": "Ошибка:", "bk.what": "Система сама кладёт копию профиля в выбранную вами папку — каждый раз, когда что-то изменилось, и в тот миг, когда вы уходите со страницы. Хранятся три поколения: беда, замеченная на другой день, лечится только вчерашним снимком. Пока стоит замок, копия уходит в папку ЗАПЕРТОЙ и открывается тем же паролем.", "bk.needFolder": "Без настоящей папки синхронизация не включается. Копия, лежащая в том же браузере, что и оригинал, — не копия: одна чистка браузера унесёт обе.", "bk.unsupported": "Этот браузер не даёт странице писать в папку на устройстве, поэтому синхронизации здесь нет — и Совет не станет её изображать. Складывать копии в тот же браузер было бы просто и было бы ложью: копия, лежащая в том же браузере, что и оригинал, — не копия, потому что одна чистка браузера унесёт обе. Остаётся выгрузка файлом: кнопка ниже. На настольном Chrome или Edge синхронизация включится сама.", "bk.permission": "Браузер просит подтвердить доступ к папке. Нажмите «Сохранить сейчас» ещё раз и разрешите.", "panel.account": "Аккаунт", "aria.identity": "Аккаунт и уход", "acc.guest": "гость на этой машине", "acc.name": "Имя в системе", "acc.nameSub": "Видно в верхней полосе и на том, что вы отправляете. Не короче двух знаков.", "acc.save": "Сохранить", "acc.saved": "Сохранено.", "acc.profile": "Профиль", "acc.signout": "Выйти, мягко", "acc.leave": "Уйти сейчас", "acc.leaveSub": "Одно нажатие: сеанс закрыт, все кэши и служебные копии стёрты, адрес и заголовок этой записи истории подменены, вкладка пытается закрыться. Написанное остаётся на месте — и остаётся запертым, если замок стоит.", "acc.wipe": "Стереть всё и уйти", "acc.wipeSub": "То же самое плюс всё написанное: заметки, письма, разговоры, Хранилище, профили. На этом устройстве от вас не остаётся ничего. Отменить нельзя.", "acc.wipeAsk": "Нажмите ещё раз, чтобы стереть всё. Это не отменяется.", "acc.truth": "Чего страница не может, и Совет не станет это изображать: список посещений браузера — не её память, и ни один сайт не вправе его читать и стирать. Сайт, умеющий стереть себя из вашей истории, умеет стереть и всё остальное. Эта страница подменяет ту запись, до которой дотягивается, — «назад» сюда не приведёт, — и стирает всё своё. Если нужно, чтобы посещения не записывались вовсе, — приватное окно браузера; убрать уже записанное можно только в самом браузере.", "panel.time": "\u0412\u0440\u0435\u043c\u044f \u0438 \u043a\u0430\u043b\u0435\u043d\u0434\u0430\u0440\u044c", "aria.time": "\u0412\u0440\u0435\u043c\u044f \u0438 \u043a\u0430\u043b\u0435\u043d\u0434\u0430\u0440\u044c", "pl.prev": "\u041d\u0430\u0437\u0430\u0434", "pl.next": "\u0412\u043f\u0435\u0440\u0451\u0434", "pl.play": "\u0418\u0433\u0440\u0430\u0442\u044c", "pl.pause": "\u041f\u0430\u0443\u0437\u0430", "pl.seek": "\u041f\u043e\u043b\u043e\u0436\u0435\u043d\u0438\u0435", "pl.pick": "\u0423\u043a\u0430\u0437\u0430\u0442\u044c \u043f\u0430\u043f\u043a\u0443", "pl.count": "\u0414\u043e\u0440\u043e\u0436\u0435\u043a: {n}", "pl.brought": "\u041c\u0443\u0437\u044b\u043a\u0430", "pl.broughtN": "\u041f\u0440\u0438\u043d\u0435\u0441\u0435\u043d\u043e \u0434\u043e\u0440\u043e\u0436\u0435\u043a: {n}", "pl.none": "\u0412 \u044d\u0442\u043e\u0439 \u043f\u0430\u043f\u043a\u0435 \u043d\u0435\u0442 \u0437\u0432\u0443\u043a\u043e\u0432\u044b\u0445 \u0444\u0430\u0439\u043b\u043e\u0432", "pl.empty.title": "\u041c\u0443\u0437\u044b\u043a\u0438 \u043f\u043e\u043a\u0430 \u043d\u0435\u0442", "pl.empty.sub": "\u0411\u0440\u0430\u0443\u0437\u0435\u0440 \u043d\u0435 \u043c\u043e\u0436\u0435\u0442 \u0441\u0430\u043c \u043f\u0440\u043e\u0441\u043c\u043e\u0442\u0440\u0435\u0442\u044c \u0432\u0430\u0448\u0435 \u0443\u0441\u0442\u0440\u043e\u0439\u0441\u0442\u0432\u043e. \u0423\u043a\u0430\u0436\u0438\u0442\u0435 \u041e\u0414\u041d\u0423 \u043f\u0430\u043f\u043a\u0443 \u2014 \u043d\u0430\u043f\u0440\u0438\u043c\u0435\u0440, Music \u0438\u043b\u0438 Download. \u041a\u043e\u0440\u0435\u043d\u044c \u043f\u0430\u043c\u044f\u0442\u0438 Android \u0432\u044b\u0431\u0440\u0430\u0442\u044c \u043d\u0435 \u0434\u0430\u0451\u0442: \u0435\u0441\u043b\u0438 \u043f\u0438\u0448\u0435\u0442 \u00abCan\u2019t use this folder\u00bb \u2014 \u0437\u0430\u0439\u0434\u0438\u0442\u0435 \u0432\u043d\u0443\u0442\u0440\u044c \u043f\u0430\u043f\u043a\u0438 \u0438 \u0432\u044b\u0431\u0435\u0440\u0438\u0442\u0435 \u0435\u0451.", "br.blank": "\u041f\u0443\u0441\u0442\u043e?", "br.blank.go": "\u041e\u0442\u043a\u0440\u044b\u0442\u044c \u0432\u043a\u043b\u0430\u0434\u043a\u043e\u0439", "br.shut.sub": "\u0412\u044b \u0441\u043a\u0430\u0437\u0430\u043b\u0438 \u0441\u0438\u0441\u0442\u0435\u043c\u0435, \u0447\u0442\u043e \u044d\u0442\u043e\u0442 \u0441\u0430\u0439\u0442 \u043d\u0435 \u0440\u0430\u0437\u0440\u0435\u0448\u0430\u0435\u0442 \u043f\u043e\u043a\u0430\u0437\u044b\u0432\u0430\u0442\u044c \u0441\u0435\u0431\u044f \u0432\u043d\u0443\u0442\u0440\u0438 \u0434\u0440\u0443\u0433\u043e\u0439 \u0441\u0442\u0440\u0430\u043d\u0438\u0446\u044b. \u0422\u0435\u043f\u0435\u0440\u044c \u043e\u043d \u0441\u0440\u0430\u0437\u0443 \u043e\u0442\u043a\u0440\u044b\u0432\u0430\u0435\u0442\u0441\u044f \u0432\u043a\u043b\u0430\u0434\u043a\u043e\u0439 \u2014 \u0438 \u044d\u0442\u043e \u043c\u043e\u0436\u043d\u043e \u043e\u0442\u043c\u0435\u043d\u0438\u0442\u044c.", "br.shut.retry": "\u041f\u043e\u043f\u0440\u043e\u0431\u043e\u0432\u0430\u0442\u044c \u0441\u043d\u043e\u0432\u0430 \u0437\u0434\u0435\u0441\u044c",
      /* ============================================================ Настройки */
      ...
      "ml.compose.honestStudio": "«Отправить» отправит письмо через службу {host} — это не наш сервер, и там письмо читается целиком, прежде чем дойдёт до почты студии. Мимо неё: напишите прямо на build@sys.baby.",
      ...
      "ml.status.emptyBody": "Сначала напишите что-нибудь — пустое письмо ничего не говорит.", "ml.compose.sendOwn": "Своей почтой", "ml.compose.ownNote": "«Своей почтой» открывает вашу почтовую программу с этим письмом: оно уходит через вашу почтовую службу, открытым текстом, а не через sys.baby.", "ml.status.ownTo": "Для своей почты нужен настоящий адрес, например name@example.com.", "ml.tag.handed": "своей почтой", "ml.delivery.handed": "Передано вашей почтовой программе {when} — письмо отправляет она, а не эта система; дошло ли оно, эта система не знает.", "ml.toast.handedTitle": "Передано вашей почте", "ml.toast.handedBody": "Открылась ваша почтовая программа с письмом. Отправляет она — копия осталась здесь, в Отправленных.",
      ...
      "fv.hand.none": "Ничего не выбрано",
      "fv.hand.noneSub": "Сначала выберите файл — папку нельзя передать одной вещью.",
      ...
      /* ============================================================= Разговор */
      "mg.sampleNotice": "Ничего никуда не ушло: эта система сама сообщений не носит. Задайте с этим человеком общее слово — тогда запечатанное сообщение можно передать любым путём, и открыть его сможет только он.", "mg.pact": "Общее слово", "mg.pactSet": "Общее слово ✓", "mg.pactWhat": "Договоритесь с этим человеком об общем слове — при встрече или голосом, но не тем же путём, каким понесёте сообщения. Им запечатываются ваши сообщения: тот, кто их несёт, — мессенджер, почта, SMS — видит только шум. Слово хранится под вашим замком и никуда не уходит.", "mg.pactPh": "общее слово, от 6 знаков", "mg.pactSave": "Запомнить слово", "mg.pactOn": "Теперь сообщения здесь можно запечатывать общим словом.", "mg.pactShort": "Общее слово — не короче {n} знаков.", "mg.seal": "Запечатать", "mg.sealTitle": "Запечатать сообщение, чтобы передать его любым путём", "mg.sealFailed": "Этот браузер не смог запечатать сообщение — ничего не изображено.", "mg.sealedOut": "Запечатано. Передайте этот текст любым путём — открыть его можно только вашим общим словом:", "mg.copy": "Скопировать", "mg.copied": "Скопировано.", "mg.copyHand": "Выделено — скопируйте вручную.", "mg.share": "Поделиться", "mg.close": "Закрыть", "mg.paste": "Открыть запечатанное", "mg.pasteWhat": "Вставьте запечатанное сообщение, полученное от этого человека, — пробелы и слова вокруг не мешают.", "mg.pasteOpen": "Открыть", "mg.pasteWrong": "Вашим общим словом это не открывается — ничего не добавлено.", "mg.pasteNoPact": "Сначала задайте общее слово — без него открывать нечем.", "mg.pasteNone": "Во вставленном нет запечатанного сообщения (sb1:…).", "mg.sealedTag": "запечатано",
      ...
      "hand.to": "Передать в…",
      "kind.вещи": "вещи", "kind.выбор": "выбор", "kind.служебное": "служебное",
      "need.диск": "диск", "need.сеть": "сеть", "need.папка": "папка", "need.почтальон": "почтальон",
      "need.свой сервер": "свой сервер", "need.чужое согласие": "чужое согласие", "need.собеседник": "собеседник",
      "verdict.есть": "есть", "verdict.нет": "нет", "verdict.нечем проверить": "нечем проверить",
      "state.живо": "живо", "state.ждёт": "ждёт", "state.нечем проверить": "нечем проверить",
      "who.ядро": "ядро", "how.чтение": "чтение", "how.запись": "запись",
      "why.solitaire": "Подарок, а не инструмент: эту комнату открывает Сундук, и пока она не подарена, стол без неё не беднее.",
      "boot.records": "{n} записей", "boot.registered": "{n} зарегистрировано", "boot.works": "{n} работ",
      "boot.priceOk": "прайс на месте", "boot.priceNo": "прайса нет",
      "boot.storageShut": "хранилище закрыто браузером", "boot.noApps": "ни одно приложение не зарегистрировалось", "boot.noPortfolio": "портфолио не подгрузилось",
      "years.b": "{n} млрд лет", "years.m": "{n} млн лет", "years.y": "{n} лет", "years.lt1": "меньше года",
      "alive.disk.noPlace": "у этой комнаты нет своего места на диске: писать некуда",
      "alive.disk.noRights": "ядро прав не поднялось, и спросить ящик комнаты не у кого",
      "alive.disk.empty": "место пустое: проверено пробной записью, и она снята",
      "alive.disk.same": "записано и прочитано обратно прямо сейчас, и лежит там то же, что лежало",
      "alive.disk.refused": "запись не вернулась: хранилище закрыто или права не дают",
      "alive.net.unknown": "браузер не сказал о сети ничего",
      "alive.net.on": "провод воткнут. отвечает ли тот, кто нужен, отсюда не видно",
      "alive.net.off": "браузер говорит, что сети нет",
      "alive.post.noList": "опись выходов не загрузилась, и спросить адрес не у кого",
      "alive.post.none": "в описи выходов не названо ни одного чужого хозяина",
      "alive.post.named": "адрес назван в описи выходов: {hosts}. примет ли он письмо сегодня — отсюда не видно",
      "alive.folder.yes": "браузер умеет открыть папку по вашему выбору",
      "alive.folder.no": "этот браузер не умеет открывать папку: работа будет только с тем, что перетащили",
      "alive.server.noList": "опись выходов не загрузилась, и спросить не у кого",
      "alive.server.none": "ни одна дверь в описи выходов не объявляет обратной дороги: адреса, по которому к вам приходило бы, сегодня нет",
      "alive.server.named": "обратная дорога объявлена в описи выходов: {hosts}",
      "alive.consent": "согласие чужого сайта показываться в рамке узнаётся только попыткой открыть его, а система не ходит наружу ради разговора о себе",
      "alive.peer": "живого человека на том конце машина не измеряет ничем: молчащий и отсутствующий отсюда неразличимы",
      "alive.noProbe": "такой нужде в системе не заведено прибора, и догадываться она не станет",
      "alive.probeFailed": "прибор не отработал: {err}",
      "why.browser": "Страница внутри страницы: большинство сайтов запрещают себя показывать так, а остальным нужна сеть — то самое, без чего система нарочно умеет жить.",
      "why.experimental": "Стенд, а не инструмент: там проверяют, возможна ли вещь. Человеку на этом этапе он не даёт ничего.",
      "why.mail": "Письма без сервера некуда отправить и неоткуда получить: приложение показывает, КАК БЫЛО БЫ, а не делает работу. Адрес почтальона в описи выходов есть; сервера, который принимал бы письма к вам, — нет.",
      "why.messenger": "Сообщения носит ваша рука: запечатанное общим словом сообщение вы сами передаёте любым путём. Есть ли на том конце живой человек, отсюда не измерить ничем.",
      "why.player": "Проигрыватель без своей музыки — картинка проигрывателя: показать ему нечего, пока вы не дали папку. Снят со стола временно (D-199); открывается по имени.",
      "why.project": "Живая система открывается не значком, а работой: карточкой в портфолио, связанным письмом или кратким описанием в Хранилище. Значок на столе вёл бы в пустую рамку.",
      "why.viewer": "Просмотр открывается вещью, а не значком: без вещи ему нечего показать, и значок обещал бы то, чего стол не выполнит.",
      "hand.done": "Передано", "hand.src.from": "Скопировано из «{room}» {d}.", "hand.src.same": "Совпадает с источником.", "hand.src.changed": "С тех пор источник изменился.", "hand.src.gone": "Источника больше нет — это последняя копия.", "hand.src.update": "Взять новую версию",
      "hand.doneSub": "Вещь теперь и там. Здесь она тоже остаётся.",
      "hand.fail": "Не передано",
      "hand.failSub": "Та комната вещь не взяла, и никто не делает вид, что взяла.",
      "nt.ph.title": "Название", "nt.ph.body": "Начните писать…",
      "nt.ph.search": "Поиск по заметкам", "nt.new": "Новая заметка",
      "nt.emptyList": "Ничего не найдено", "nt.more": "Показать ещё {n}",
      "nt.empty": "Заметок пока нет. Напишешь — останется у тебя: ни одна строка отсюда не уходит.",
      "nt.newNote": "Новая заметка",
      "ec.files.one": "1 выброшенный файл", "ec.files.many": "выброшенных файлов: {n}", "ec.file.folder": "папка", "ec.file.file": "файл", "ec.silenceAsk": "Удалить насовсем? Обратной дороги нет.", "ec.restore": "Вернуть",
    ...
    ee: {
      ...
      "cc.transparency": "Vähem läbipaistvust",
      "cc.volume": "Helitugevus", "cc.brightness": "Heledus", "cc.language": "Keel",
      "cc.fullscreen": "T\u00e4isekraan", "cc.wallpaper": "Taust", "cc.controls": "Akna nupud", "cc.controls.left": "Vasakul", "cc.controls.right": "Paremal",
      "cc.shortcuts": "Klaviatuuri otseteed", "cc.layouts": "Akende paigutused",
      "cc.health": "Seisund", "cc.incognito": "Peidetud töölaud",
      ...
      "note.placeholder": "Kirjuta midagi…", "note.sheetQ": "Mis on praegu oluline?",
      "note.aria": "Märge",
      ...
      "note.due.rightNow": "praegu", "note.due.inMin": "{n} min pärast", "note.due.endOf": "{day} lõpuks",
      ...
      "p.saveCurrent": "Salvesta praegune",
      ...
      "h.measured": "Elus, mõõdetud — mitte hinnatud",
      ...
      "aria.minimizeAll": "sys.baby private OS — puhasta laud: ahenda kõik aknad",
      "aria.tip": "Näita vihjet", "cc.tip": "Näita vihjet",
      "aria.cc": "Juhtimiskeskus",
      ...
      "net.gone": "Internetti pole", "net.nothingChanged": "Midagi ei muutunud. Kõik kirjutatu on siin, sest see oli alati siin. Võrku vajab ainult Brauser — teiste lehed ei ole meie omad, et neid hoida.", "net.stillHere": "Võrku pole — süsteem on siin", "cc.connection": "Ühendus", "cc.online": "Võrgus",
      ...
      "lock.duress": "Hädasõna", "lock.duressOff": "Eemalda hädasõna",
      "lock.duressField": "teine sõna — avab teise süsteemi",
      "lock.duressWhat": "Teine sõna avab teise, tervikliku süsteemi: oma töölaua, oma märkmed. Miski ei ütle, kas see sõna on olemas — ei silt, ei viivitus, ei ükski rida kettal. Just see vaikus ongi ainus põhjus, miks see toimib.", "lock.stays": "Lukustatud olekus jääb kettal loetavaks ainult see:", "lock.stays.sysbaby.lock.v1": "luku kirje ise: sool ja šifrite nimed — ilma nendeta võtit ei tuletata", "lock.stays.sysbaby.activeProfile": "millist selle brauseri profiili avada — number, mitte nimi", "lock.stays.sysbaby.authed": "märk „sisselogimine tehtud“ — jah või ei", "lock.stays.folder": "viide sinu varukoopiate kaustale koos kausta nimega — koopiad selles on pitseeritud", "lock.keyFile": "Teise võtme fail", "lock.keyFileWhat": "Sellel lukul on vaja ka teise võtme faili — et vahetada sõna, seada teine sõna, luua taastekood või lukk eemaldada.", "lock.spare": "Taastekood", "lock.spareWhat": "32-märgiline kood avab sinu põhisüsteemi, kui sõna on ununenud — ka ilma teise võtme failita. Seda näidatakse üks kord. Kirjuta see paberile ja hoia sellest seadmest eraldi: kellel on kood, sellel on süsteem. Uus kood tühistab vana; kasutatud kood enam ei tööta.", "lock.spareOn": "Taastekood loodud {d}.", "lock.spareNone": "Taastekoodi veel pole.", "lock.spareDone": "Kirjuta see kohe üles — teist korda seda ei näidata.", "lock.spareSave": "Salvesta failina", "lock.spareFile": "sys.baby — taastekood\n\n{code}\n\nLoodud {d}. Avab sinu põhisüsteemi selles seadmes, kui sõna on ununenud — ilma sõnata ja ilma teise võtmeta. Töötab üks kord. Hoia seadmest eraldi: kellel on kood, sellel on süsteem.\n", "lock.forgot": "Unustasid sõna?", "lock.codeWhat": "Sisesta taastekood ja uus sõna. Kood avab põhisüsteemi ja kulub seejärel ära. Kui teine võti oli seatud, see eemaldatakse ning sellega loodud teine sõna kaob.", "lock.code": "Taastekood", "lock.codeNew": "Uus sõna", "lock.codeAgain": "Uus sõna uuesti", "lock.codeOpen": "Ava koodiga", "lock.codeWrong": "See kood lukku ei ava. Midagi ei muutunud.", "lock.codeUsed": "Taastekood on ära kasutatud. Loo uus konto aknas.",
      "lock.duressSame": "teine sõna peab esimesest erinema", "lock.duressDone": "Valmis.",
      "lock.costMem": "Pluss {mb} MiB mälu iga katse kohta — seda, mida graafikakaardid ja eriotstarbelised kiibid jagada ei saa.",
      "lock.strong": "Tugevda mäluga",
      "lock.strongWhat": "See lukk venitab parooli ainult ajaga. Tugevdamine lisab Argon2id: iga arvamiskatse vajab ka 64 MiB oma mälu, mis teeb arvamise graafikakaartidel ja eriotstarbelistel kiipidel mitu korda kallimaks. Märkmed jäävad nagu on — ümber ehitatakse ainult uksed. Kui seadsid hädasõna, kirjuta see, kui küsitakse: ilma selleta läheb selle taga olev süsteem kaduma.",
      "lock.strongAsk": "Kui seadsid hädasõna, kirjuta see siia ja vajuta uuesti. Kui ei seadnud, jäta tühjaks — teine uks muutub müraks.",
      "lock.strongDone": "Tugevdatud: nüüd maksab iga katse lisaks ajale ka mälu.",
      "lock.duressWrong": "See hädasõna ei ava oma ust — midagi ei muudetud.",
      "lock.hw": "Seadme võti",
      "lock.hwWhat": "Seo lukk selle seadme turvakiibiga. Pärast seda avaneb see ainult parooli JA sinu sõrmejälje, näo või seadme PIN-iga. Saladus ei lahku kunagi kiibist: süsteem saab ainult selle vastuse ühele küsimusele ja seda vastust ei kirjutata kuhugi. Varastatud ketas koos piilutud parooliga on ikka mitte midagi. Kui seade kaob, avab süsteemi ainult taastekood — ja see eemaldab seadme võtme.",
      "lock.hwOn": "See lukk küsib ka seadme võtit.",
      "lock.hwNone": "Seadme võtit pole seotud.",
      "lock.hwCant": "See brauser ei oska siin seadme võtit hoida.",
      "lock.hwAdd": "Seo seadme võti",
      "lock.hwOff": "Eemalda seadme võti",
      "lock.hwDone": "Seotud. Nüüdsest küsib lukk ka sinu seadet.",
      "lock.hwRemoved": "Eemaldatud. Lukk avaneb jälle parooliga.",
      "lock.hwNoPrf": "See brauser või seade ei oska vastata küsimusele, mida lukk vajab (WebAuthn PRF). Midagi ei muudetud.",
      "lock.hwFailed": "Seade ei kinnitanud — midagi ei muudetud.",
      "lock.hwNeed": "See lukk küsib ka seadet: pärast parooli kinnita sõrmejälje, näo või PIN-iga.",
      "lock.hwNo": "Seadme võti ei vastanud — lukk jääb suletuks. Ilma seadmeta avab selle ainult taastekood.",
      "mood.studio": "Stuudio", "mood.aurora": "Virmalised", "mood.sunset": "Loojang", "mood.ocean": "Ookean", "mood.mono": "Mono", "mood.daylight": "Päevavalgus", "mood.therapy": "Valgusteraapia", "mood.feast": "Pidu", "mood.ember": "Hõõgus", "mood.own": "Sinu oma", "mood.midnight": "Kesköö", "mood.garden": "Aed", "mood.lavender": "Lavendel", "mood.honey": "Mesi", "menu.tidyDesk": "Korista laud", "menu.bringThing": "Too asi\u2026", "menu.hideFab": "Eemalda see nupp", "part.fab": "Kiirtegevuste nupp", "br.back": "Tagasi", "br.fwd": "Edasi", "br.again": "V\u00e4rskenda", "br.bookmark": "Salvesta leht", "br.bookmarked": "Salvestatud", "br.openTab": "Ava p\u00e4ris kaardil", "br.placeholder": "Aadress v\u00f5i otsing", "br.frameTitle": "Leht", "br.saved": "Salvestatud", "br.recent": "Hiljutine", "br.empty.title": "Midagi pole veel avatud", "br.empty.sub": "Sisesta aadress ja vajuta Enter. Paljud saidid keelavad end teise lehe sees n\u00e4idata \u2014 parempoolne nool avab aadressi alati p\u00e4ris kaardil.", "note.day.title": "See p\u00e4ev on sinu", "note.day.body": "Kuup\u00e4ev on kirjas \u2014 kirjuta edasi, mis sel p\u00e4eval juhtub.", "lock.again": "Korda parooli", "lock.mismatch": "Paroolid ei kattu \u2014 midagi ei lukustatud.", "lock.failed": "Lukustada ei \u00f5nnestunud \u2014 midagi ei muudetud.", "lock.remove": "Eemalda lukk", "lock.removed": "Lukk on eemaldatud. S\u00f5nad on taas lahti.", "lock.warn": "Kui unustad selle parooli ja sul pole taastekoodi, ei saa kirjutatut taastada. Koopiat pole ühelgi serveril — selle nimel kõik ongi. Loo allpool taastekood ja hoia seda paberil.", "lock.key": "Teine võti — vali fail", "lock.keyNeed": "See süsteem küsib ka teist võtit. Ilma failita ei tuletata paroolist midagi.", "alarm.title": "Äratus", "alarm.now": "{time} — nüüd.", "alarm.passed": "{time} möödus {ago}.", "alarm.agoMin": "{n} min tagasi", "alarm.agoHour": "{n} t tagasi", "alarm.agoDay": "{n} p tagasi", "alarm.set": "Äratus pandud", "alarm.off": "Äratus eemaldatud", "alarm.arm": "Ärata mind sel ajal", "set.desktop.wake": "Ärata ka väljaspool vahekaarti", "set.desktop.wakeSub": "Ainus asi, mis sellest süsteemist üldse välja läheb. Sisselülitamine küsib brauserilt luba; siis ilmub äratus telefoni teadetesse — kus seda loeb igaüks, kes kõrval seisab. Vaikimisi väljas ja ise sisse ei lülitu.", "lock.cost": "{n} käiku · {ms} ms. Nii palju maksab ÜKS katse — ja neid on vaja miljoneid.", "lock.sealed": "{n} ümbrikku sellel kettal. Ühtegi neist siit lugeda ei saa.", "lock.ask": "Parool", "lock.open": "Ava", "lock.wrong": "See parool ei ava.", "lock.locked": "Lukus", "lock.title": "Lukusta kõik", "lock.insecure": "Sellel aadressil lukk ei tööta. Päris krüptograafia annab brauser lehele ainult turvalise ühenduse (https) kaudu. Ava sys.baby üle https-i — lukk ilmub ja midagi muud ei muutu.", "aria.turboOn": "Turbo on sees — puuduta, et kaadrid tagasi anda", "cc.turbo": "Turbo", "lock.state.none": "Ei ole lukus", "lock.state.armed": "Lukus — avatud selleks seansiks", "lock.what": "Üks parool sulgeb kogu süsteemi: iga märkme, iga kirja, iga vestluse, Hoidla ja toa välimuse. Kettale ei jää midagi loetavat — isegi mitte hoitavate asjade nimesid.", "lock.accounts": "Mitu kontot? Üks parool avab need kõik selles seadmes. Lukk seisab hoidla ees, mitte ühe konto ees — ja kuni ta on kinni, ei paista isegi see, mitu kontot sul on.", "lock.new": "Parool", "lock.set": "Lukusta kõik", "lock.now": "Lukusta kohe", "lock.idle": "Lukusta, kui liikumist pole", "lock.idle.never": "Mitte kunagi", "lock.idle.min": "{n} min", "lock.idle.hour": "{n} h", "lock.idleWhat": "Nii kaua ei ühtki klõpsu, klahvi, rullimist ega puudutust \u2014 ja seanss sulgub: uks tuleb tagasi, sõnad jäävad ümbrikutesse.", "lock.change": "Muuda parooli", "lock.changed": "Parool on muudetud. Midagi ei krüpteeritud uuesti — vahetati vaid võtit hoidev ümbrik.", "lock.old": "Praegune parool", "lock.short": "Vähemalt neli märki.", "lock.busy": "Arvutan — see viivitus on siin meelega.", "auth.lock.title": "Kõik selle saab sulgeda ühe sõnaga", "auth.lock.body": "sys.baby'l ei ole serverit. Kirjutatu jääb sellesse seadmesse — see tähendab, et keegi ei loe seda sinu eest ega sinu asemel. Üks võtmekeeramine ja kogu süsteem läheb ümbrikesse: märkmed, kirjad, vestlused, isegi sinu seinte värv. Keegi teine seda taastada ei saa — koopiat pole kellelgi; päästa saab ainult sinu enda taastekood paberil, mida lukk pakub. Seda saab teha kohe või ükskõik millisel hilisemal päeval — ava ülaribal on alati olemas.", "auth.lock.now": "Sea kohe", "auth.lock.later": "Hiljem, ülaribalt", "bk.title": "Koopiad", "bk.folder": "Kaust", "bk.noFolder": "pole valitud", "bk.last": "Viimane koopia", "bk.never": "veel pole", "bk.sealed": "lukus", "bk.choose": "Vali kaust", "bk.change": "Vaheta kausta", "bk.on": "Sees", "bk.off": "Väljas", "bk.saveNow": "Salvesta kohe", "bk.exportNow": "Laadi koopia failina alla", "bk.err": "Viga:", "bk.what": "Süsteem paneb sinu profiili koopia sinu valitud kausta — iga kord, kui midagi muutus, ja sel hetkel, kui lehelt lahkud. Hoitakse kolme põlvkonda: päev hiljem märgatud häda saab parandada ainult eilse koopiaga. Kui lukk on peal, läheb koopia kausta LUKUSTATULT ja avaneb sama parooliga.", "bk.needFolder": "Ilma päris kaustata sünkroonimine sisse ei lülitu. Koopia, mis elab samas brauseris kui originaal, ei ole koopia: üks brauseri puhastus viib mõlemad.", "bk.unsupported": "See brauser ei luba lehel seadme kausta kirjutada, seega sünkroonimist siin ei ole — ja nõukogu seda teesklema ei hakka. Koopiate panek samasse brauserisse oleks lihtne ja oleks vale: koopia, mis elab samas brauseris kui originaal, ei ole koopia, sest üks brauseri puhastus viib mõlemad. Jääb fail, mille laadid ise alla nupuga allpool. Töölaua Chrome'is või Edge'is lülitub sünkroonimine ise sisse.", "bk.permission": "Brauser palub kausta juurdepääsu kinnitada. Vajuta «Salvesta kohe» veel kord ja luba.", "panel.account": "Konto", "aria.identity": "Konto ja lahkumine", "acc.guest": "külaline selles masinas", "acc.name": "Nimi süsteemis", "acc.nameSub": "Näha ülaribal ja sellel, mida saadad. Vähemalt kaks märki.", "acc.save": "Salvesta", "acc.saved": "Salvestatud.", "acc.profile": "Profiil", "acc.signout": "Logi välja, pehmelt", "acc.leave": "Lahku kohe", "acc.leaveSub": "Üks puudutus: seanss suletakse, kõik vahemälud ja teeninduskoopiad kustutatakse, selle ajalookirje aadress ja pealkiri asendatakse ning kaart üritab sulguda. Kirjutatu jääb paigale — ja jääb lukku, kui lukk on peal.", "acc.wipe": "Kustuta kõik ja lahku", "acc.wipeSub": "Kõik eelnev pluss kõik kirjutatu: märkmed, kirjad, vestlused, Hoidla, profiilid. Selles seadmes ei jää sinust midagi. Tagasi ei saa.", "acc.wipeAsk": "Vajuta veel kord, et kõik kustutada. Seda ei saa tagasi võtta.", "acc.truth": "Mida leht ei saa teha ja mida nõukogu teesklema ei hakka: brauseri külastuste loend ei ole selle lehe mälu ja ükski sait ei tohi seda lugeda ega kustutada. Sait, mis oskaks end sinu ajaloost kustutada, oskaks kustutada ka kõike muud. See leht asendab kirje, milleni ta ulatub — «tagasi» siia ei too — ja kustutab kõik omast. Kui on vaja, et külastusi üldse ei salvestataks, kasuta privaatakent; juba salvestatu saab eemaldada ainult brauseris endas.", "panel.time": "Aeg ja kalender", "aria.time": "Aeg ja kalender", "pl.prev": "Eelmine", "pl.next": "J\u00e4rgmine", "pl.play": "Esita", "pl.pause": "Paus", "pl.seek": "Asukoht", "pl.pick": "Vali kaust", "pl.count": "Lugusid: {n}", "pl.brought": "Muusika", "pl.broughtN": "Toodud lugusid: {n}", "pl.none": "Selles kaustas pole helifaile", "pl.empty.title": "Muusikat veel pole", "pl.empty.sub": "Brauser ei saa ise sinu seadet l\u00e4bi vaadata. Vali \u00dcKS kaust \u2014 n\u00e4iteks Music v\u00f5i Download. Androidi m\u00e4lu juurt valida ei saa: kui \u00fctleb \u201cCan\u2019t use this folder\u201d, mine kausta sisse ja vali see.", "br.blank": "T\u00fchi?", "br.blank.go": "Ava kaardil", "br.shut.sub": "Sa \u00fctlesid s\u00fcsteemile, et see sait ei luba end teise lehe sees n\u00e4idata. Nutu avaneb ta alati p\u00e4ris kaardil \u2014 ja selle saab tagasi v\u00f5tta.", "br.shut.retry": "Proovi siin uuesti",
      /* ============================================================== Seaded
      ...
      "ml.compose.honestStudio": "Saatmine saadab kirja teenuse {host} kaudu — see pole meie server ja kiri on seal loetav, enne kui jõuab stuudio postkasti. Mööda minnes kirjuta otse aadressile build@sys.baby.",
      ...
      "ml.status.emptyBody": "Kirjutage kõigepealt midagi — tühi kiri ei ütle midagi.", "ml.compose.sendOwn": "Oma postiga", "ml.compose.ownNote": "«Oma postiga» avab sinu postiprogrammi selle kirjaga: see lahkub sinu enda postiteenuse kaudu, avatud tekstina, mitte sys.baby kaudu.", "ml.status.ownTo": "Oma posti jaoks on vaja päris aadressi, näiteks nimi@example.com.", "ml.tag.handed": "oma postiga", "ml.delivery.handed": "Antud sinu postiprogrammile {when} — kirja saadab tema, mitte see süsteem; kas see kohale jõudis, see süsteem ei tea.", "ml.toast.handedTitle": "Antud sinu postile", "ml.toast.handedBody": "Avanes sinu postiprogramm kirjaga. Saadab tema — koopia jäi siia, Saadetud kausta.",
      ...
      "fv.hand.none": "Midagi pole valitud",
      "fv.hand.noneSub": "Vali kõigepealt fail — kausta ei saa ühe asjana üle anda.",
      ...
      /* =============================================================== Sosin */
      "mg.sampleNotice": "Midagi ei saadetud: see süsteem ise sõnumeid ei kanna. Seadke selle inimesega ühine sõna — siis võib pitseeritud sõnum liikuda mis tahes teed ja avada saab selle ainult tema.", "mg.pact": "Ühine sõna", "mg.pactSet": "Ühine sõna ✓", "mg.pactWhat": "Leppige selle inimesega kokku ühine sõna — kohtudes või häälega, mitte sama teed pidi, mida sõnumid lähevad. Sellega pitseeritakse teie sõnumid: see, kes neid kannab — sõnumirakendus, e-post, SMS —, näeb ainult müra. Sõna hoitakse sinu luku all ega saadeta kuhugi.", "mg.pactPh": "ühine sõna, vähemalt 6 märki", "mg.pactSave": "Jäta sõna meelde", "mg.pactOn": "Nüüd saab siinseid sõnumeid ühise sõnaga pitseerida.", "mg.pactShort": "Ühine sõna peab olema vähemalt {n} märki.", "mg.seal": "Pitseeri", "mg.sealTitle": "Pitseeri sõnum, et saata see mis tahes teed", "mg.sealFailed": "See brauser ei suutnud sõnumit pitseerida — midagi ei teeseldud.", "mg.sealedOut": "Pitseeritud. Saada see tekst mis tahes teed — avada saab seda ainult teie ühise sõnaga:", "mg.copy": "Kopeeri", "mg.copied": "Kopeeritud.", "mg.copyHand": "Valitud — kopeeri käsitsi.", "mg.share": "Jaga", "mg.close": "Sulge", "mg.paste": "Ava pitseeritud sõnum", "mg.pasteWhat": "Kleebi sellelt inimeselt saadud pitseeritud sõnum — tühikud ja sõnad ümber ei sega.", "mg.pasteOpen": "Ava", "mg.pasteWrong": "Sinu ühise sõnaga see ei avane — midagi ei lisatud.", "mg.pasteNoPact": "Sea kõigepealt ühine sõna — ilma selleta pole millegagi avada.", "mg.pasteNone": "Kleebitus pole pitseeritud sõnumit (sb1:…).", "mg.sealedTag": "pitseeritud",
      ...
      "hand.to": "Anna üle…",
      "kind.вещи": "asjad", "kind.выбор": "valikud", "kind.служебное": "süsteemi oma",
      "need.диск": "ketas", "need.сеть": "võrk", "need.папка": "kaust", "need.почтальон": "postiljon",
      "need.свой сервер": "oma server", "need.чужое согласие": "teise saidi nõusolek", "need.собеседник": "elav inimene",
      "verdict.есть": "on", "verdict.нет": "ei", "verdict.нечем проверить": "pole millegagi kontrollida",
      "state.живо": "töötab", "state.ждёт": "ootab", "state.нечем проверить": "kontrollimatu",
      "who.ядро": "tuum", "how.чтение": "lugemine", "how.запись": "kirjutamine",
      "why.solitaire": "Kingitus, mitte tööriist: selle toa avab Laegas, ja kuni seda pole kingitud, pole laud ilma selleta vaesem.",
      "boot.records": "{n} kirjet", "boot.registered": "{n} registreeritud", "boot.works": "{n} tööd",
      "boot.priceOk": "hinnakiri kohal", "boot.priceNo": "hinnakirja pole",
      "boot.storageShut": "brauser on salvestusruumi sulgenud", "boot.noApps": "ükski rakendus ei registreerunud", "boot.noPortfolio": "portfoolio ei laadinud",
      "years.b": "{n} miljardit aastat", "years.m": "{n} miljonit aastat", "years.y": "{n} aastat", "years.lt1": "alla aasta",
      "alive.disk.noPlace": "sellel toal pole kettal oma kohta: pole kuhu kirjutada",
      "alive.disk.noRights": "õiguste tuum ei tulnud üles ja toa kasti pole kelleltki küsida",
      "alive.disk.empty": "koht on tühi: kontrollitud proovikirjega, mis seejärel eemaldati",
      "alive.disk.same": "kirjutatud ja tagasi loetud just praegu, ja seal on see, mis seal enne oli",
      "alive.disk.refused": "kirje ei tulnud tagasi: salvestusruum on suletud või õigused ei luba",
      "alive.net.unknown": "brauser ei öelnud võrgu kohta midagi",
      "alive.net.on": "kaabel on sees. kas see, keda vajad, vastab — seda siit ei näe",
      "alive.net.off": "brauser ütleb, et võrku pole",
      "alive.post.noList": "väljapääsude register ei laadinud ja aadressi pole kelleltki küsida",
      "alive.post.none": "väljapääsude register ei nimeta ühtegi võõrast omanikku",
      "alive.post.named": "aadress on väljapääsude registris nimetatud: {hosts}. kas see täna kirja vastu võtab — seda siit ei näe",
      "alive.folder.yes": "brauser oskab avada sinu valitud kausta",
      "alive.folder.no": "see brauser ei oska kausta avada: töö piirdub sellega, mis sisse lohistati",
      "alive.server.noList": "väljapääsude register ei laadinud ja pole kelleltki küsida",
      "alive.server.none": "ükski uks väljapääsude registris ei kuuluta tagasiteed: täna pole aadressi, kuhu midagi sinuni jõuaks",
      "alive.server.named": "tagasitee on väljapääsude registris kuulutatud: {hosts}",
      "alive.consent": "kas võõras sait nõustub end raamis näitama, selgub alles avamiskatsel, ja süsteem ei käi väljas enda pärast rääkimas",
      "alive.peer": "elavat inimest teises otsas ei mõõda siin miski: vaikiv ja puuduv paistavad seestpoolt ühesugused",
      "alive.noProbe": "sellele vajadusele pole süsteemis mõõteriista ja ta ei hakka arvama",
      "alive.probeFailed": "mõõteriist ei töötanud: {err}",
      "why.browser": "Leht lehe sees: enamik saite keelab end nii näidata ja ülejäänud vajavad võrku — just seda, milleta süsteem on meelega ehitatud elama.",
      "why.experimental": "Katsepink, mitte tööriist: seal kontrollitakse, kas asi on võimalik. Inimesele ei anna see praegusel etapil midagi.",
      "why.mail": "Ilma serverita pole kirjadel kuhugi minna ega kusagilt tulla: rakendus näitab, KUIDAS OLEKS, mitte tööd ennast. Postiljoni aadress on väljapääsude registris olemas; serverit, mis sinule kirju vastu võtaks, ei ole.",
      "why.messenger": "Sõnumeid kannab sinu käsi: ühise sõnaga pitseeritud sõnumi saadad sa ise mis tahes teed. Kas teises otsas on elav inimene, ei saa siit millegagi mõõta.",
      "why.player": "Mängija ilma oma muusikata on mängija pilt: tal pole midagi näidata, kuni sa talle kausta ei anna. Ajutiselt laualt maha võetud (D-199); avaneb nime järgi.",
      "why.project": "Elav süsteem avaneb tööst, mitte ikoonist: portfoolio kaardist, seotud kirjast või Hoidla lühikirjeldusest. Ikoon laual viiks tühja raami.",
      "why.viewer": "Vaatur avaneb asjast, mitte ikoonist: ilma asjata pole tal midagi näidata, ja ikoon lubaks seda, mida laud ei täida.",
      "hand.done": "Antud üle", "hand.src.from": "Kopeeritud kohast «{room}» {d}.", "hand.src.same": "Ühtib allikaga.", "hand.src.changed": "Allikas on vahepeal muutunud.", "hand.src.gone": "Allikat enam pole — see on viimane koopia.", "hand.src.update": "Võta uus versioon",
      "hand.doneSub": "See on nüüd ka seal. Siia see samuti jääb.",
      "hand.fail": "Ei antud üle",
      "hand.failSub": "See tuba ei võtnud seda vastu, ja keegi ei teeskle vastupidist.",
      "nt.ph.title": "Pealkiri", "nt.ph.body": "Alustage kirjutamist…",
      "nt.ph.search": "Otsi märkmetest", "nt.new": "Uus märge",
      "nt.emptyList": "Midagi ei leitud", "nt.more": "Näita veel {n}",
      "nt.empty": "Märkmeid veel pole. Kirjuta — ja see jääb sinu juurde: ükski rida siit ei lahku.",
      "nt.newNote": "Uus märge",
      "ec.files.one": "1 kustutatud fail", "ec.files.many": "kustutatud faile: {n}", "ec.file.folder": "kaust", "ec.file.file": "fail", "ec.silenceAsk": "Kustutada jäädavalt? Tagasiteed ei ole.", "ec.restore": "Taasta",
  ...
  function showTip(tip) {
    ...
    /* Человек вызвал подсказку — извещения уступают ей место (см.
       ...
       ── А ПРИШЕДШАЯ САМА — НЕ УСТУПАЕТ, А НЕ ВЫТЕСНЯЕТ · D-252 ─────────
       Здесь стоял безусловный вызов, и подсказка, пришедшая по своему
       расписанию, убивала извещение, которое человек ещё читал: ровно
       обратное правилу, записанному строкой выше. Закон toast-surface-check
       поймал это тем, что его извещение исчезало на 587-й миллисекунде.
       Правило одно: важнее то, что человек позвал сам. Позвал подсказку —
       извещения уходят. Пришла сама — она ждёт, а не отнимает. */
    if (hintRequested && window.sbToastsYield) window.sbToastsYield();
  ...
  function scheduleTips() {
    ...
    function cycle() {
      /* Извещение на экране — подсказка, пришедшая сама, ждёт своей очереди. */
      var busy = doc.querySelector("#toastLayer .toast");
      if (doc.visibilityState === "hidden" || busy || (window.sbAnyPanelOpen && window.sbAnyPanelOpen())) {
