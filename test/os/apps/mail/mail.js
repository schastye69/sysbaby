...
(function () {
  ...
  var EMPTY_TEXT = {
    inbox: "Nothing in the inbox. Replies land here.",
    starred: "Nothing starred. Star a message to keep it within reach.",
    sent: "Nothing sent yet.",
    drafts: "No drafts. Anything you start writing waits here.",
  ...
  var FOLDERS = [
    { id: "inbox", label: "Inbox", icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><path d="M4 13.5 6.2 6.2A1.4 1.4 0 0 1 7.5 5.2h9a1.4 1.4 0 0 1 1.3 1L20 13.5v4.3a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1v-4.3Z"/><path d="M4 13.5h4l1.2 2.2h5.6l1.2-2.2h4"/></svg>' },
    { id: "starred", label: "Starred", icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><path d="m12 4.8 2.3 4.7 5.2.8-3.8 3.6.9 5.1-4.6-2.4-4.6 2.4.9-5.1L4.5 10.3l5.2-.8L12 4.8Z"/></svg>' },
    { id: "sent", label: "Sent", icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><path d="M20 4 4 10.4l6.3 2.4L20 4Z"/><path d="M20 4 13.4 20l-3.1-7.2L20 4Z"/></svg>' },
    { id: "drafts", label: "Drafts", icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><path d="M5 5.2h9.5L19 9.6V19a.9.9 0 0 1-.9.9H5a.9.9 0 0 1-.9-.9V6.1A.9.9 0 0 1 5 5.2Z"/><path d="M14.5 5.2v4.4H19"/></svg>' },
    { id: "trash", label: "Trash", icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round"><path d="M4.5 7h15M9.5 7V5.2a1 1 0 0 1 1-1h3a1 1 0 0 1 1 1V7M6.5 7l1 12.2a1 1 0 0 0 1 .9h7a1 1 0 0 0 1-.9L17.5 7"/></svg>' }
  ];
  ...
  function timeAgo(ts) {
    ...
    if (diff >= 7 * 86400000) {
      ...
      catch (err) { return new Date(t).toDateString(); }
    }
  ...
  function starMarkup(m) {
    return '<button type="button" class="ml-star' + (m.starred ? " on" : "") + '" data-star="' + m.id + '" title="' + (m.starred ? "Unstar" : "Star") + '" aria-label="Star">' +
  ...
  function deliveryTag(m, folder) {
    if (folder === "sent" && m.delivery) {
      if (m.delivery.state === "delivered") return '<span class="ml-tag delivered">✓ delivered</span>';
      if (m.delivery.state === "local") return '<span class="ml-tag local">local</span>';
    }
    if (folder === "drafts" && m.composeMode === "studio") return '<span class="ml-tag studio">to the studio</span>';
    return "";
  }
    ...
    return '<div class="ml-row' + (m.unread ? " unread" : "") + (m.id === selectedId ? " active" : "") + '" data-id="' + m.id + '">' +
      ...
      '<div class="ml-row-main">' +
        '<div class="ml-row-top">' +
          '<span class="ml-row-who">' + (m.unread ? '<i class="ml-dot"></i>' : "") + esc(who || "") +
            (folder === "drafts" ? '<span class="ml-draft-tag">Draft</span>' : "") + deliveryTag(m, folder) + "</span>" +
          '<span class="ml-row-time">' + esc(timeAgo(m.ts)) + "</span>" +
        "</div>" +
        '<div class="ml-row-subject">' + esc(m.subject || "(no subject)") + "</div>" +
        '<div class="ml-row-snippet">' + esc(m.snippet || "") + "</div>" +
      "</div>" +
    "</div>";
  }
  ...
  function emptyMarkup(win) {
    ...
    var text = needle ? "Nothing matches “" + esc(needle) + "”" : esc(EMPTY_TEXT[win._mailFolder] || "");
  ...
  function linkKindLabel(kind) {
    ...
    if (kinds && kinds[kind] && kinds[kind].label) return kinds[kind].label;
    return String(kind || "");
  }
  ...
  function connectedMarkup(id) {
    ...
    var note = isStudio
      ? "This letter is real — and so is everything it points to."
      : "This message is a sample. Everything it points to is real and opens.";
    var rows = links.map(function (link, idx) {
      ...
      return '<button type="button" class="ml-link' + (live ? " live" : "") + '" data-link="' + idx + '">' +
        ...
        '<span class="ml-link-text"><span class="ml-link-title">' + esc(link.title || "") + "</span>" +
          '<span class="ml-link-sub">' + esc(live ? "Open now" : (link.sub || "")) + "</span></span>" +
        '<span class="ml-link-mark">' + (live ? '<i class="ml-live-dot"></i>' : "→") + "</span>" +
      "</button>";
    }).join("");
    return '<div class="ml-connected"><div class="ml-connected-head">Connected</div>' +
  ...
  function readingMarkup(win) {
    ...
    if (!m || !inView) return '<div class="ml-read-empty"><p>Select a message</p></div>';

    var isSent = m.folder === "sent";
    var contact = isSent ? (m.to || "") : (m.from || "");
    var contactLink = contact && contact !== "You"
      ? '<button type="button" class="ml-contact" data-contact="' + esc(contact) + '" title="See everything from this contact">' + esc(contact) + "</button>"
      : esc(contact);

    var meta = isSent
      ? "To: " + contactLink
      : "From: " + contactLink + " &lt;" + esc(m.fromAddr || "") + "&gt;";

    var actions;
    if (m.folder === "trash") {
      actions = starMarkup(m) +
        '<button type="button" class="ml-btn" id="mlRestore">Restore</button>' +
        '<button type="button" class="ml-btn danger" id="mlPurge">Delete forever</button>';
    } else if (m.folder === "drafts") {
      actions = '<button type="button" class="ml-btn primary" id="mlContinue">Continue editing</button>' +
        starMarkup(m) +
        '<button type="button" class="ml-btn" id="mlDelete">Delete</button>';
    } else {
      actions = '<button type="button" class="ml-btn" id="mlReply">Reply</button>' +
        '<button type="button" class="ml-btn" id="mlForward">Forward</button>' +
        '<button type="button" class="ml-btn" id="mlToNote" title="Save this message as a Scribble note">To Scribble</button>' +
        starMarkup(m) +
        '<button type="button" class="ml-btn" id="mlDelete">Delete</button>';
    }
    ...
    if (m.folder === "sent" && m.delivery) {
      if (m.delivery.state === "delivered") {
        deliveryLine = '<div class="ml-delivery ok">Delivered to ' + esc(STUDIO_ADDR) + " — a real email, sent " + esc(timeAgo(m.delivery.at)) +
          (m.channel ? ". Reply requested by " + esc(m.channel) + (m.contact ? " · " + esc(m.contact) : "") : "") + ".</div>";
      } else if (m.delivery.state === "local") {
        deliveryLine = '<div class="ml-delivery quiet">Local letter — it lives in this browser and was never transmitted anywhere.</div>';
      }
    }

    return '<div class="ml-read">' +
      '<h2 class="ml-read-subject">' + esc(m.subject || "(no subject)") + "</h2>" +
      '<div class="ml-read-meta">' + meta + ' <span class="ml-sep">·</span> ' + esc(timeAgo(m.ts)) + "</div>" +
      deliveryLine +
      '<div class="ml-read-actions">' + actions + "</div>" +
      '<div class="ml-read-body">' + esc(m.body || "") + "</div>" +
  ...
  function composeMarkup(win) {
    ...
    var channelOptions = CHANNELS.map(function (c) {
      return '<option value="' + esc(c) + '"' + (c === channel ? " selected" : "") + ">" + esc(c) + "</option>";
    }).join("");

    return '<div class="ml-compose">' +
      '<div class="ml-compose-head"><span id="mlComposeFrom">New letter · From ' + esc(myAddress()) + "</span>" +
        '<button type="button" class="ml-compose-x" id="mlComposeClose" title="Close" aria-label="Close">×</button></div>' +
      /* Two kinds of letter, and the interface never lets them blur:
         one truly leaves, one truly stays. */
      '<div class="ml-mode">' +
        '<button type="button" class="ml-mode-btn' + (studio ? " active" : "") + '" data-compose-mode="studio">To the studio</button>' +
        '<button type="button" class="ml-mode-btn' + (!studio ? " active" : "") + '" data-compose-mode="local">Local</button>' +
        '<span class="ml-mode-note">' + (studio
          ? "really sent — a human reads it"
          : "stays in this browser") + "</span>" +
      "</div>" +
      (studio
        ? '<div class="ml-field ml-to-fixed">To: <b>' + esc(STUDIO_ADDR) + "</b><span> · real delivery</span></div>"
        : '<input type="text" id="composeTo" class="ml-field" placeholder="To (a name is fine — it&#39;ll become name@sys.baby)" value="' + esc(d.to || "") + '" autocomplete="off">') +
      '<input type="text" id="composeSubject" class="ml-field" placeholder="Subject" value="' + esc(d.subject || "") + '" autocomplete="off">' +
      ...
      '<textarea id="composeBody" class="ml-field ml-body" placeholder="' + (studio ? "What are we building together?" : "Write your message…") + '">\n' + esc(d.body || "") + "</textarea>" +
      (studio
        ? '<div class="ml-reply-row">' +
            '<label class="ml-reply-label">How should we answer?</label>' +
      ...
      '<div class="ml-compose-foot">' +
        '<span class="ml-honest">' + (studio
          ? "Send posts a real email to the studio inbox — through the same relay as the order form on the site."
          : "Local letters never leave this device.") + "</span>" +
        '<button type="button" class="ml-btn primary" id="mlSend">' + (studio ? "Send to the studio" : "Send · local") + "</button>" +
      "</div>" +
    "</div>";
  }
  ...
  function channelPlaceholder(channel) {
    switch (channel) {
      ...
      case "Phone": return "+372 …";
      case "Telegram": return "@username";
      case "WhatsApp": return "+372 …";
      case "Signal": return "+372 … or username";
      case "URL": return "https://…";
      default: return "Where do we find you?";
    }
  }
        ...
        '<aside class="ml-folders">' +
          FOLDERS.map(function (f) {
            ...
            return '<button type="button" class="ml-folder' + (win._mailFolder === f.id ? " active" : "") + '" data-folder="' + f.id + '">' +
              '<span class="ml-folder-icon">' + f.icon + "</span><span>" + esc(f.label) + "</span>" + badge + "</button>";
          }).join("") +
        "</aside>" +
        '<div class="ml-mid">' +
          '<div class="ml-toolbar">' +
            '<button type="button" class="ml-btn primary" id="mailCompose">Compose</button>' +
            '<button type="button" class="ml-btn studio" id="mailComposeStudio" title="A real letter to a real inbox">To the studio</button>' +
            '<input type="search" class="ml-search" id="mlSearch" placeholder="Search mail" autocomplete="off" value="' + esc(win._mailSearch) + '">' +
          "</div>" +
  ...
  function bindReading(win, host) {
    ...
    if (toNote && m) {
      toNote.addEventListener("click", function () {
        ...
        toast("Saved to Scribble", "The message is a note now — a record you can work with.");
      });
    }
    ...
    if (del && m) {
      del.addEventListener("click", function () {
        ...
        pushBadge();
        render(win);
        toast("Mail", "Message moved to Trash");
      });
    }
    ...
    if (purge && m) {
      purge.addEventListener("click", function () {
        if (!window.confirm('Permanently delete "' + (m.subject || "this message") + "\"? This can't be undone.")) return;
  ...
  function sendToStudio(win, host, fields) {
    ...
    composeStatus(host, "Posting to the studio relay…");
    ...
    var fail = function (why) {
      ...
      if (live) {
        ...
        if (b) { b.disabled = false; b.textContent = "Send to the studio"; }
        composeStatus(live, why + " Your letter is kept in Drafts — nothing was lost, and nothing pretends it was sent.", "warn");
      }
      toast("Not delivered", "The relay could not be reached. The letter waits in Drafts.");
    };

    if (navigator.onLine === false) { fail("You are offline."); return; }
    ...
    fetch(RELAY, { method: "POST", body: fd, headers: { Accept: "application/json" } })
      ...
      .then(function (delivered) {
        if (!delivered) { fail("The mail relay refused or did not answer."); return; }
        ...
        selectedId = id;
        render(win);
        toast("Delivered", "A real email is now in the studio inbox. We answer on the channel you chose.");
    ...
    if (send) {
      send.addEventListener("click", function () {
        ...
        if (fields.mode === "studio") {
          if (!fields.body.trim()) { composeStatus(host, "Write something first — an empty letter says nothing.", "warn"); return; }
        ...
        selectedId = null;
        render(win);
        toast("Kept locally", "This letter lives in this browser only — nothing left your device.");
      });
    }
  }
  ...
  document.addEventListener("sysbaby:username-changed", function () {
    ...
    if (line) line.textContent = "New message · From " + myAddress();
  });
