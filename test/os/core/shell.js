...
(function () {
  ...
  var MOODS = [
    ...
    /* ── УГЛИ · решение D-187 ─────────────────────────────────────────────
       Это подарок, и он сделан не по заявке, а по обстоятельствам.

       Основатель строит эту систему НОЧЬЮ. 27.08.2026, в третьем часу, он
       написал: «пора отдохнуть. хотя мне просыпаться через 3 часа и идти на
       работу» — и попросил у Совета небольшой подарок к утру.

       Все восемь комнат до этой сделаны для дня. Самая тусклая — Моно, и даже
       она втрое ярче того, что нужно глазу в три часа ночи; светотерапия и
       праздник светят почти в полную силу. Человек, работающий по ночам,
       сидел в дневной комнате, потому что другой не было.

       УГЛИ — комната для тех часов, когда всё остальное погашено. Три её
       свойства ИЗМЕРЕНЫ, а не заявлены (см. закон):
         · САМАЯ ТЁМНАЯ. Её самый яркий миг (0.20) не ярче, чем самый тусклый
           миг любой другой комнаты с постоянным кольцом: у Моно, прежней
           тусклейшей, нижний край ровно 0.20. Суточная комната в сравнение не
           входит и не должна: ночью она темнеет сама — в этом её работа.
         · САМАЯ НЕПОДВИЖНАЯ. Полоса тона шириной в шесть градусов (13–19°) —
           уже всех прочих, и кольцо в 52 минуты — длиннее всех. Угли не
           мерцают: они держат тепло и почти не двигаются.
         · ТЁПЛАЯ ВСЯ ЦЕЛИКОМ. Полоса не выходит за тёплый конец круга ни в
           одной точке кольца — в отличие от комнат, которые круг проходят.

       ЧЕГО ЗДЕСЬ НЕ СКАЗАНО НАРОЧНО: Совет не обещает, что от этой комнаты
       лучше спится, и не выдаёт её за лечение. Сказано только измеримое.
       Что с этим делать в три часа ночи, человек решает сам.

       И ещё одно, вслух. Комната называется Углями, а не Пеплом. Угли — это
       то, что горит, когда пламени уже не видно.
       Охраняется tools/ember-check.mjs. */
    { id: "ember", name: "Ember", hue: 16, span: 6, cycle: 52 * 60000, room: [0.10, 0.20], sat: 56 }
  ];
  function moodDef(id) {
  ...
  window.sbRoomPalette = function (moodId) {
    /* ── НАЗВАЛИ КОМНАТУ — ПОЛУЧАЕТЕ ЕЁ · решение D-191 ────────────────────
       Здесь стояло: сперва ПРИМЕНЁННЫЙ свет, и только если его нет — свет
       названной комнаты. То есть довод moodId молча не работал НИКОГДА, пока
       на столе горит любая комната: спросив палитру Углей, звавший получал
       палитру Студии. Закон room-and-seam поймал это на новой комнате
       («совпали со Студией: ember»), но дефекту столько же лет, сколько самой
       функции, и касался он всех.
       Теперь просто: назвали комнату — считаем её свет; не назвали — берём
       применённый. */
    var w = moodId
      ? window.sbRoomLight(Date.now(), moodId)
      : ((window.sbRoomNow && window.sbRoomNow()) || window.sbRoomLight());
    ...
    /* ── ТЁМНАЯ КОМНАТА ДАЁТ ТЁМНОЕ ПОЛЕ (D-191) ──────────────────────────
       Палитра брала у света тон и насыщенность, а УРОВЕНЬ — сколько света в
       комнате вообще — не брала вовсе. Оттого Угли (0.10–0.20) светили в поле
       ровно как Студия (0.30–0.46), и самая тёмная комната системы выглядела
       на столе как самая обычная.
       Поправка только ВНИЗ: комната ярче опорной ничего не получает, комната
       темнее — темнеет и полем. Так ни одна из прежних комнат не сдвинулась,
       а тёмные (Угли, ночная суточная) стали наконец тёмными. */
    var dim = Math.min(0, (w.level - 0.30) * 60);
    var rgb = function (h, lAdd) {
      var c = hexToRgb(hslHex(h, sat, Math.max(6, Math.min(90, driftLight(h) + (lAdd || 0) + dim))));
      return [c.r, c.g, c.b];
    };
    ...
    gate.innerHTML =
      '<div class="vg-box">' +
        ...
        /* ── ЗАЯВЛЕНИЕ, А НЕ ОПРАВДАНИЕ · решение D-190 ────────────────────
           ...
           encrypted system, baby. nothing can leave us».
           ЗДЕСЬ СТОЯЛ АБЗАЦ, ОБЪЯСНЯВШИЙ, ЧТО ИМЕННО ЗАПЕРТО. Объяснение
           верное — и не на своём месте: дверь не оправдывается, дверь
           заявляет. Всё, что надо объяснить, объясняется В ОКНЕ АККАУНТА, где
           замок ставят: там и цена, и шифр, и предупреждение о потерянном
           пароле. Здесь — одна строка, и она правда по построению: наружу
           действительно не уходит ничего.
